...
test('E2E: Invite → Activate → my-rights → DB → PATCH → erneut my-rights (App-only-Paket)', async (t) => {
  ...
  await t.test('POST /invites: user_id erzwingt Ziel-E-Mail (falsche body-email wird ignoriert)', async () => {
    const targetId = randomUUID();
    const targetEmail = `target-ma-${targetId.slice(0, 8)}@cc-cockpit-e2e.local`;
    const wrongEmail = `wrong-body-${randomUUID().slice(0, 8)}@cc-cockpit-e2e.local`;
    await store.insertUser({
      id: targetId,
      email: targetEmail,
      passwordHash: hashPassword('Temp!2026'),
      name: 'E2E Target MA',
      globalRole: 'MITARBEITER',
    });
    await store.updateUserCompany(targetId, seedFirmaId);
    const { res, body } = await apiV1('post', '/invites', adminToken, {
      jsonBody: {
        email: wrongEmail,
        user_id: targetId,
    ...
    assert.equal(res.status, 201, JSON.stringify(body));
    assert.equal(
      String(body.data?.invite?.email || '').toLowerCase(),
      targetEmail.toLowerCase(),
      'Invite muss E-Mail des Ziel-users speichern',
    );
    const row = await store.getPendingCockpitInviteByEmail(targetEmail);
    assert.ok(row, 'pending invite für Ziel-E-Mail');
    assert.notEqual(
      String(row.email).toLowerCase(),
      wrongEmail.toLowerCase(),
    );
  });

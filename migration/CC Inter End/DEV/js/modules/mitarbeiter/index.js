...
//      Gespeichert in localStorage (Session-State, operativ)
...
(function() {
  ...
  var VERF_KEY = 'cc_intern_ma_verfuegbar_v1';

  // ── Verfügbarkeits-Status-Store ──────────────────────────────────────
  // Leichtgewichtig: nur localStorage, kein Server (operativer Tages-Status)
  var MA_VERF = {};
  ...
  function loadVerfuegbar() {
    try {
      var raw = localStorage.getItem(VERF_KEY);
      MA_VERF = raw ? JSON.parse(raw) : {};
    } catch(e) { MA_VERF = {}; }
  }

  function saveVerfuegbar() {
    try { localStorage.setItem(VERF_KEY, JSON.stringify(MA_VERF)); } catch(e) {}
  }

  // ── Quick-Status setzen ──────────────────────────────────────────────
  window.maSetVerfuegbar = function(maId, status) {
    if (!maId) return;
    MA_VERF[maId] = status || 'verfuegbar';
    saveVerfuegbar();
  ...
  window.maOpenVerfuegbarPicker = function(maId, anchorEl) {
    closeVerfuegbarPicker();
    var current = MA_VERF[maId] || 'verfuegbar';
    ...
    window.renderMitarbeiter = function() {
      ...
      setTimeout(function() {
        ...
        cards.forEach(function(card) {
          ...
          if (!match) return;
          var maId = match[1];
          var status = MA_VERF[maId] || 'verfuegbar';
  ...
  function init() {
    ...
    console.info('[Mitarbeiter] Quick-Verfügbarkeit geladen — ' + Object.keys(MA_VERF).length + ' Einträge');
  }
  ...
  window.MitarbeiterService = {
    getVerfuegbar: function(maId) { return MA_VERF[maId] || 'verfuegbar'; },

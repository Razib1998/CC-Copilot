...
window.AuftragChecklisten = (function() {
  ...
  function vonVorlageAnwenden(auftragId, step, vorlagenId) {
    ...
    // Existierende Checkliste nicht überschreiben wenn bereits Einträge vorhanden
    var existing = a.schritte[step].checkliste || [];
    if (existing.length > 0) {
      if (!confirm('Schritt "' + step + '" hat bereits ' + existing.length + ' Prüfpunkte.\nVorlage ZUSÄTZLICH hinzufügen?')) return;
    }

    // Punkte aus Vorlage übernehmen
  ...
  function checklisteLeeren(auftragId, step) {
    ...
    if (!confirm('Checkliste für "' + step + '" wirklich leeren?')) return;

...
function renderAuftragVerwaltung(){
  ...
  // Filter
  var data = AUFTRAEGE.filter(function(a){
    // Archiv-Tab: nur archivierte zeigen
    if(auVerwFilter==='archiv') return !!a.archiv;
    // Alle anderen Tabs: archivierte ausblenden
    if(a.archiv) return false;
    if(auVerwFilter==='offen'        && a.step==='abgeschlossen') return false;
    if(auVerwFilter==='abgeschlossen'&& a.step!=='abgeschlossen') return false;
    if(auVerwFilter==='rechnung'     && !(a.step==='abgeschlossen'&&a.rechnung==='offen')) return false;
    if(q && !(
  ...
  // Stats (archivierte aus normalen Zahlen raus)
  var aktiv  = AUFTRAEGE.filter(function(a){ return !a.archiv; });
  ...
  var reOff  = aktiv.filter(function(a){ return a.step==='abgeschlossen'&&a.rechnung==='offen'; }).length;
  var archiviert = AUFTRAEGE.filter(function(a){ return !!a.archiv; }).length;
...
function renderChecklisten(){
  const liste = document.getElementById('cl-vorlagen-liste'); if(!liste) return;
  const total = CL_VORLAGEN.length;
  const aktiv = CL_VORLAGEN.filter(v=>v.aktiv).length;
  const punkte = CL_VORLAGEN.reduce((a,v)=>a+v.punkte.length,0);
  const st=document.getElementById('cl-stat-total');  if(st) st.textContent=total;
  const sa=document.getElementById('cl-stat-aktiv');  if(sa) sa.textContent=aktiv;
  const sp=document.getElementById('cl-stat-punkte'); if(sp) sp.textContent=punkte;
  ...
  liste.innerHTML = CL_VORLAGEN.map(v=>{
    const isAkt = clAktivId===v.id;
    const lBg = lightMap[v.farbe]||'var(--blue-l)';
    const pflicht = v.punkte.filter(p=>p.kat==='pflicht').length;
    const foto    = v.punkte.filter(p=>p.kat==='foto').length;
    return '<div onclick="clOpenVorlage(\''+v.id+'\')" style="display:flex;align-items:center;gap:10px;padding:12px 16px;border-bottom:1px solid var(--border);cursor:pointer;background:'+(isAkt?lBg:'#fff')+';border-left:3px solid '+(isAkt?v.farbe:'transparent')+';">'
      ...
      +'<div style="flex:1;min-width:0;">'
        ...
        +'<div style="font-size:11px;color:var(--text2);margin-top:1px;">'+v.punkte.length+' Punkte'
          +(pflicht?' · '+pflicht+' Pflicht':'')+(foto?' · 📷 '+foto+' Foto':'')+'</div>'
      +'</div>'
      +'<span class="bdg '+(v.aktiv?'bg':'bgr')+'">'+(v.aktiv?'Aktiv':'Inaktiv')+'</span>'
    +'</div>';
  }).join('') || '<div style="padding:20px;text-align:center;color:var(--text3);">Noch keine Vorlagen</div>';
}

function clOpenVorlage(id){
  clAktivId=id;
  renderChecklisten();
  const v=CL_VORLAGEN.find(x=>x.id===id); if(!v) return;
...
function clDeleteVorlage(id){
  ...
  CL_VORLAGEN=CL_VORLAGEN.filter(v=>v.id!==id);
  clAktivId=null;
  renderChecklisten();
  const body=document.getElementById('cl-detail-body');
  const ph=document.getElementById('cl-detail-ph');
...
function clEditPunkt(vorlageId, idx){
  const v=CL_VORLAGEN.find(x=>x.id===vorlageId); if(!v) return;
...
function clSavePunkt(){
  ...
  const v=CL_VORLAGEN.find(x=>x.id===clPunktVorlageId); if(!v) return;
...
function clDeletePunkt(vorlageId, idx){
  if(!confirm('Möchten Sie diesen Checklisten-Punkt wirklich löschen?')) return;
  const v=CL_VORLAGEN.find(x=>x.id===vorlageId); if(!v) return;
...
function renderKanban(){
  ...
  const reList = AUFTRAEGE.filter(a=>!a.archiv&&a.step==='abgeschlossen'&&a.rechnung==='offen');

  // ── Rechnung schreiben Banner ── (entfernt — Lexware-Queue in Rechnungen-Seite)
  let reBanner='';

  // ── Stats ──
  let statsHtml='<div class="stats" style="grid-template-columns:repeat(5,1fr);margin-bottom:14px;">'
    ...
    cards.forEach(function(a){
      ...
      cols+='<div style="padding:10px 12px 6px;padding-left:'+(a.urgent||istUeberf||istHeute?'16':'12')+'px;cursor:pointer;" onclick="openAuftragDetail(\''+a.id+'\')">'
        +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1px;">'
          ...
          +'<div style="display:flex;align-items:center;gap:4px;">'+chatBdg+(isAbg?'<span>✅</span>':'')+'<button onclick="event.stopPropagation();loescheAuftrag(\''+a.id+'\')" style="background:none;border:none;cursor:pointer;padding:2px 4px;opacity:.4;font-size:13px;line-height:1;" title="Auftrag löschen">🗑</button></div>'
        +'</div>'
      ...
      if(isAbg){
        ...
        cols+='<div style="padding:7px 10px;border-top:1px solid var(--border);background:var(--green-l);">'
          ...
          +'<div style="display:flex;gap:4px;">'+renderRechnungButtons(a.id,a.rechnung)+'</div></div>';
        ...
        var laufend=ZEIT_AKTIV[a.id+'_'+step];
...
function schrittFertig(id, maId){
  ...
  if(nextStep && nextStep!=='abgeschlossen' && a.schritte[nextStep]){
...
function loescheAuftrag(id){
  ...
  var a = AUFTRAEGE[idx];
  if(!confirm('Auftrag '+id+' ('+a.kunde+' · '+a.fz+') wirklich löschen?\nDieser Vorgang kann nicht rückgängig gemacht werden.')){ return; }
  AUFTRAEGE.splice(idx, 1);
  renderKanban();
  saveAuftraege();
  showToast('🗑 '+id+' gelöscht');
}

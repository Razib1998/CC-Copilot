...
// ═══════════════════════════════════════════════════════
// PERSISTENZ — automatisches Speichern / Laden
// Projekte & Benachrichtigungen bleiben beim Neuladen erhalten.
// ═══════════════════════════════════════════════════════
const MF_PERSIST_KEY = 'mf_state_v1';

function mfSaveState() {
  if (typeof window === 'undefined' || typeof localStorage === 'undefined') return;
  try {
    const data = JSON.stringify({ projects: state.projects, notifs: state.notifs });
    localStorage.setItem(MF_PERSIST_KEY, data);
  } catch(e) {
    console.warn('[MesseFlow] Speichern fehlgeschlagen:', e);
  }
}

function mfLoadState() {
  if (typeof window === 'undefined' || typeof localStorage === 'undefined') return;
  // ── CC Cockpit-Logik: echte localStorage-Daten haben IMMER Vorrang ──────
  // Wenn localStorage reale Projekte enthält → diese laden (überschreibt Demo).
  // Nur wenn localStorage leer ist UND MF_USE_DEMO_DATA=true → Demo bleibt.
  try {
    const raw = localStorage.getItem(MF_PERSIST_KEY);
    if (!raw) return; // leer → Demo-State (aus mfBuildInitialState) bleibt
    const saved = JSON.parse(raw);
    if (saved && Array.isArray(saved.projects) && saved.projects.length > 0) {
      // Echte Daten vorhanden → Demo überschreiben
      state.projects = saved.projects;
      console.log('[MesseFlow] Cockpit: echte Projekte aus localStorage geladen (' + saved.projects.length + ')');
    }
    if (saved && Array.isArray(saved.notifs)) {
      state.notifs = saved.notifs;
    }
  } catch(e) {
    console.warn('[MesseFlow] Laden fehlgeschlagen, Storage wird zurückgesetzt:', e);
    try { localStorage.removeItem(MF_PERSIST_KEY); } catch(e2) {}
  }
}

// Beim Laden der Seite sofort Daten wiederherstellen
if (typeof window !== 'undefined') mfLoadState();
...
function isUserGesperrt(user) {
  if (window.MF_TEST_MODE === true) return false; // Test-Modus: keine Sperren
...
function isProjGesperrt(projektId, userId) {
  if (window.MF_TEST_MODE === true) return false; // Test-Modus: keine Projektsperren
  const p = state.projects.find(x => x.id === projektId);
...
const ALLE_RECHTE_AN = { sehen: true, upload: true, freigabe: true, angebote: true, preise: true, kommentieren: true, loeschen: true, exportieren: true, einladen: true };

function getProjRechte(userId, projektId) {
  // Test-Modus: alle Rechte für alle Benutzer vollständig freischalten
  if (window.MF_TEST_MODE === true) return { ...ALLE_RECHTE_AN };
  const user = USERS.find(u => u.id === userId);
...
/** Nur Admins: Druck-Flow zurücksetzen (z. B. nach Fehløbertragung), danach wieder bearbeitbar. */
...
// positionen = [{ name, breite, hoehe, menge, einheit, material, bemerkung }]
function addZusatzbestellungPositionen(projectId, positionen) {
  const project = state.projects.find(p => p.id === projectId);
  if (!project || !positionen || !positionen.length) return 0;
  positionen.forEach((pos, i) => {
    const breite = pos.breite ? String(pos.breite).trim() : '';
    const hoehe  = pos.hoehe  ? String(pos.hoehe).trim()  : '';
    const einheit = pos.einheit || 'mm';
    const bestellmass = (breite && hoehe) ? `${breite} × ${hoehe} ${einheit}` : '';
    const wand = {
      id: 'w' + (Date.now() + i),
      name: pos.name || `Position ${project.waende.length + i + 1}`,
      datei: null,
      dateien: [],
      bestellmass,
      dateiMass: '',
      ...
      istZusatzbestellung: true,
      angelegtAm: nowStr(),
      angelegtVon: currentUserId,
    };
    project.waende.push(wand);
  });
  mfSaveState();
  refreshProjectUI();
...
// ── MesseFlow State Namespace ────────────────────────────────────────────────
// window.state war ein Namenskonflikt mit FUSA (beide setzen window.state).
// Ab jetzt: window.MesseFlowState ist der offizielle State-Accessor für MesseFlow.
// window.state bleibt als Alias für Rückwärtskompatibilität (wird später entfernt).
window.MesseFlowState = state;
window.state = state; // TODO: Alias – nach vollständiger Migration entfernen
window.getP = getP;
window.getW = getW;
...
window.addZusatzbestellungPositionen = addZusatzbestellungPositionen;

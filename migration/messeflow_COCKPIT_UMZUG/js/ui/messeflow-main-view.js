...
async function onDateiAusgewaehlt(pid, wid, input){
  ...
  try {
    ...
    // Modal-Status direkt aus clientStatus — effektivePruefSlot würde alten
    // w0.status (z.B. 6 = blockiert von vorheriger Datei) fälschlicherweise einrechnen
    const slotModal = clientStatus; // 'ok' | 'warnung' | 'fehler'
    ...
    // ── Risiko-Button: zeigen wenn echte Warnungen vorhanden ────────────────
    const risikoBtn = document.getElementById(`risiko-btn-${wid}`);
    if (risikoBtn) {
      if (hatEchteWarnung) {
        risikoBtn.style.display = '';
        // Warnungen im Button-Text konkret nennen
        const warnListe = (e.pruefung||[])
          .filter(z => z.status === 'warnung' || z.status === 'fehler')
          .map(z => `${z.titel}: ${z.wert}`).join(' · ');
        risikoBtn.textContent = `⚠ Trotzdem hochladen – auf eigenes Risiko (${warnListe || 'Warnungen vorhanden'})`;
 ...
 */
function messeflowCockpitBoot(cockpitUserId, container) {
  // 1. Login-Gate verstecken (existiert in Cockpit ggf. nicht → kein Fehler)
  const gate = document.getElementById('mf-login-gate');
  if (gate) gate.style.display = 'none';

  // 2. Mount-Container setzen
  if (container) {
    const view = document.getElementById('view');
    if (!view) {
      // Falls kein #view → container direkt als Ziel registrieren
      container.id = 'view';
    }
  }

  // 3. Cockpit-User als aktiven MesseFlow-User setzen
  const userId = cockpitUserId || (typeof USERS !== 'undefined' && USERS[0] ? USERS[0].id : 'u_celal');

  // 4. Normal-Boot ohne Login-Check aufrufen
  ...
  console.log('[MF] Cockpit-Boot abgeschlossen — User:', userId);
}

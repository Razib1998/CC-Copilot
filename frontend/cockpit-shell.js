...
import {
  ...
  deriveShellUiAccess,
  loadMyRights,
} from "./core/access/cc-my-rights.js";
...
import {
  ...
  getFusaAppProject,
} from "./modules/fusa/fusa-project-context.js";
...
function logMaHeaderHideDebug(renderVariant) {
  const root =
    typeof document !== "undefined"
      ? document.getElementById("cockpit-root")
      : null;
  const ui = typeof window !== "undefined" ? window.CC_SHELL_UI_ACCESS : null;
...
function syncMitarbeiterAppOnlyShellLayoutClass() {
  ...
  root.classList.toggle(
    "ckp-shell-layout--mitarbeiter-app-only",
    isMitarbeiterAppOnlyActive(),
  );
}
...
async function resolveShellRightsBundleForShell() {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      console.warn("[REFRESH_FLOW]", "resolveShellRightsBundleForShell ok", {
        attempt,
      });
      return { bundle, ui };
    } catch (e) {
      ...
      if (attempt === 0) {
        ...
        console.warn(
          "[REFRESH_FLOW]",
          "tryRefreshAccessToken after my-rights fail",
          { refreshed },
        );
  ...
  throw lastErr instanceof Error
    ? lastErr
    : new Error("Rechte konnten nicht geladen werden");
}
...
function enforceMitarbeiterAppOnlyShellState(contextLabel) {
  ...
  if (activeModule === APP_ONLY_MODULE && activeView === APP_ONLY_VIEW)
    return true;
  activeModule = APP_ONLY_MODULE;
  activeView = APP_ONLY_VIEW;
  console.warn(
...
function renderDashboardNeuDropdownHtml() {
  const entries = [
    ...
    {
      label: "Neue Schadensmeldung",
      mod: "fusa",
      key: "fusa_schaeden",
      pending: "new_schaden",
    },
    {
      label: "Neuer Fahrzeugauftrag",
      mod: "fusa",
      key: "fusa_auftraege",
      pending: "new_fahrzeugauftrag",
    },
    {
      label: "Neuer Kunde",
      mod: "fusa",
...
function navigateCrossModule(targetMod, targetKey, pendingNeu) {
  ...
  if (
    ...
    !(targetMod === "cockpit" && targetKey === "kalender")
  ) {
    ccwInvalidateKalenderEventCache();
  }
...
function attachDashboardNeuMenu(mount) {
  ...
  function menuItemsEls() {
    return [...pop.querySelectorAll(".ckp-neu-menuitem")].filter(
...
async function buildHtmlForActiveView(rid) {
  ...
  if (isAppOnlyShellLocked() && activeModule === "cockpit") {
    console.warn(
      "[APP_ONLY_BLOCK_DESKTOP]",
      "buildHtmlForActiveView-cockpit-branch",
      {
        activeModule,
        activeView,
      },
  ...
  if (activeModule !== "cockpit") {
    ...
    if (activeModule === "ccintern") {
      ...
      content =
        ccProjectCtxBanner +
        (await renderCcInternActiveViewHtml(activeView, label));
      if (rid !== contentRenderSeq) return "";
      ...
      try {
        ...
        const projects = Array.isArray(pr.projects)
      ...
      switch (activeView) {
        ...
        case "fusa_dokumente":
          ...
          break;
        case "fusa_benutzer":
          content = await renderModuleUsersReadonlyHtml({
            title: "Benutzer",
            moduleKey: "fusa",
          });
          break;
        case "fusa_rollen":
          content = await renderModuleRoleTemplatesReadonlyHtml({
  ...
  if (activeView === "projekte" || activeView === "auftraege") {
    if (isAppOnlyShellLocked()) {
      console.warn(
        "[APP_ONLY_BLOCK_DESKTOP]",
        "cockpit-projekte-auftraege-fallback",
        {
          activeModule,
          activeView,
        },
      );
      enforceMitarbeiterAppOnlyShellState(
        "cockpit-projekte-auftraege-fallback-block",
      );
    } else {
...
async function renderActiveViewIntoContent() {
  ...
  if (typeof document !== "undefined" && !fusaNavigateDocListenerBound) {
    ...
    document.addEventListener("ccw:cross-module-navigate", (ev) => {
      const d =
        ev && "detail" in ev ? /** @type {CustomEvent} */ (ev).detail : null;
      if (!d || typeof d !== "object") return;
      const mod = String(
        /** @type {Record<string, unknown>} */ (d).module || "",
      ).trim();
      const view = String(
        /** @type {Record<string, unknown>} */ (d).view || "",
      ).trim();
      if (!mod || !view) return;
      navigateCrossModule(mod, view);
    });
    document.addEventListener("ccw:fusa-navigate", (ev) => {
      ...
      if (!view) return;
      const sm = /** @type {Record<string, unknown>} */ (d)
        .fusaSchaedenMeldenFahrzeugId;
      if (sm != null && String(sm).trim() !== "")
        CCState.set("fusaSchaedenMeldenFahrzeugId", String(sm).trim());
      else CCState.set("fusaSchaedenMeldenFahrzeugId", null);
      const an = /** @type {Record<string, unknown>} */ (d)
        .fusaAuftragNeuFahrzeugId;
      if (an != null && String(an).trim() !== "")
        CCState.set("fusaAuftragNeuFahrzeugId", String(an).trim());
      else CCState.set("fusaAuftragNeuFahrzeugId", null);
      const ow = /** @type {Record<string, unknown>} */ (d)
        .fusaAuftragNeuOpenWizard;
      if (
        "fusaAuftragNeuOpenWizard" in /** @type {Record<string, unknown>} */ (d)
      ) {
        CCState.set("fusaAuftragNeuOpenWizard", ow === true);
      } else {
        CCState.set("fusaAuftragNeuOpenWizard", false);
      }
      const inn = /** @type {Record<string, unknown>} */ (d)
        .fusaAuftragNeuInternNotiz;
      if (inn != null && String(inn).trim() !== "")
        CCState.set("fusaAuftragNeuInternNotiz", String(inn).trim());
  ...
  const partialKalRequested =
    cockpitKalenderPartialDomNext && isSharedKalenderViewActive();
  cockpitKalenderPartialDomNext = false;

  const kalDynEl =
    partialKalRequested && mount
      ? /** @type {HTMLElement|null} */ (
          mount.querySelector("#ccw-cockpit-kal-dynamic")
        )
      : null;
  ...
  if (partialKalRequested && kalDynEl instanceof HTMLElement) {
    ...
    if (tDom && lastKalenderRenderPerf) {
      ...
      const totalMs =
        Math.round((performance.now() - lastKalenderRenderPerf.t0) * 100) / 100;
  ...
  <p class="ckp-mock-note">Ansicht konnte nicht gerendert werden. Bitte Konsole (F12) prüfen oder eine andere Seite in der Sidebar wählen.</p>
    ...
    if (activeView === APP_ONLY_VIEW) {
      ...
      logMaHeaderHideDebug(
        leakedHeader instanceof HTMLElement &&
          String(leakedHeader.textContent || "").trim() !== ""
          ? "leaked-header"
          : "styl-c-dom",
      );
    }
  ...
  if (isSharedKalenderViewActive()) {
    ...
    kalenderNowLineTimer = setInterval(
      () => updateCockpitKalenderNowLine(mount),
      60000,
    );
  }
  ...
  if (activeModule === "fusa") {
    ...
    if (activeView === "fusa_auftraege") {
      attachFusaAuftraegeViewHandlers(mount, () =>
        renderActiveViewIntoContent(),
      );
    }
    ...
    if (activeView === "fusa_schaeden") {
      ...
      attachFusaSchadenDetailHandlers(mount, () =>
        renderActiveViewIntoContent(),
      );
    }
    ...
    if (activeView === "fusa_quartalsabrechnung") {
      attachFusaQuartalsabrechnungHandlers(mount, () =>
        renderActiveViewIntoContent(),
      );
    }
  }
  if (activeModule === "ccintern") {
    attachCcInternActiveViewHandlers(mount, () =>
      renderActiveViewIntoContent(),
    );
  }
}
...
function tryNavigateToNavKey(key) {
  ...
  if (
    isSharedKalenderViewActive() &&
    !(activeModule === "cockpit" && key === "kalender")
  ) {
    ccwInvalidateKalenderEventCache();
  }
  activeView = key;
...
function onSidebarClick(ev) {
  const t = ev.target;
  const btn =
    t && typeof t.closest === "function" ? t.closest("[data-nav-key]") : null;
  if (!btn) return;
...
function onMainClick(ev) {
  ...
  const shellNeuBtn =
    typeof t.closest === "function"
      ? t.closest(".ckp-header .ccds-btn-neu:not(.ckp-neu-trigger)")
      : null;
  ...
  const openFusaBtn =
    typeof t.closest === "function"
      ? t.closest("[data-ccw-open-fusa-view]")
      : null;
  ...
  const viewKalBtn =
    typeof t.closest === "function" ? t.closest("[data-ccw-kal-view]") : null;
  if (
    viewKalBtn &&
    isSharedKalenderViewActive() &&
  ...
  const navBtn =
    typeof t.closest === "function" ? t.closest("[data-ccw-kal-nav]") : null;
  if (
    navBtn &&
    isSharedKalenderViewActive() &&
  ...
  const btn =
    typeof t.closest === "function" ? t.closest("[data-nav-key]") : null;
  if (!btn) return;
...
function renderInviteTokenPanelHtml(token, payload, errMsg) {
  ...
  <button type="button" class="ckp-api-login-submit" data-ccw-invite-dismiss>Zurück</button>
    ...
    const inv =
      payload && payload.invite && typeof payload.invite === "object"
        ? payload.invite
        : {};
...
async function mountCockpitShellAuthenticated(sidebar, content, main) {
  ...
  if (shellUi?.isMitarbeiterAppOnlyShell === true) {
    ...
    console.warn(
      "[APP_ONLY_FORCE]",
      "mountCockpitShellAuthenticated",
      "vorher:",
      routeBefore,
      "nachher:",
      {
    ...
    console.warn("[APP_ONLY_ROUTE]", "mountCockpitShellAuthenticated", {
      activeModule,
      activeView,
    });
    ...
    logAppOnlyDebug(
      "mountCockpitShellAuthenticated-route-desktop",
      null,
      shellUi,
      {
        activeModule,
  ...
  try {
    enforceMitarbeiterAppOnlyShellState(
      "mountCockpitShellAuthenticated-pre-render",
    );
...
export async function mountCockpitShell() {
  ...
  if (!sidebar || !content || !main) {
    console.error(
      "[CockpitShell] fehlt #cockpit-sidebar, #cockpit-content oder #cockpit-main",
    );
    return;
  }
  ...
  const inviteTok = isInviteActivationRoute()

...
async function openAuftragDetail(id){
  ...
  console.log('UI CHECKLISTE BEFORE RENDER', a.checklisten);
    ...
    _alleDateien.forEach(function(f,fi){
      ...
      _uploadsHTML +=
        ...
        +'onmouseover="this.style.background=\'#F8FBFF\'" onmouseout="this.style.background=\'#fff\'">'
          ...
          +'<div style="text-align:center;"><button onclick="ccDeleteUpload(\''+a.id+'\',\''+f._src+'\','+fi+')" style="background:none;border:none;cursor:pointer;color:#FF3B30;font-size:17px;padding:2px 4px;line-height:1;" title="Löschen">🗑</button></div>'
        +'</div>';
    });
    _uploadsHTML += '</div>';
  }
...
/** Führende User-UUID für Kommentar-Speicherung: Mobile zuerst, sonst Cockpit-User, sonst CC_AKTIV_MA (ohne Namens-/Kürzel-Rückschluss). */
function ccKommentarAutorUuidFuerSpeichern(){
  if(typeof MOB_MA_ID !== 'undefined' && MOB_MA_ID != null && String(MOB_MA_ID).trim() !== ''){
    return String(MOB_MA_ID).trim();
  }
  ...
  if(typeof window !== 'undefined' && window.CC_AKTIV_MA && window.CC_AKTIV_MA.maId != null && String(window.CC_AKTIV_MA.maId).trim() !== ''){
    return String(window.CC_AKTIV_MA.maId).trim();
  }
  return '';
}
...
function sendKommentar(auftragId, text, istFrage){
  ...
  for(si = 0; si < AUFTRAEGE.length; si++){
    sx = AUFTRAEGE[si];
    if(sx && (sx.id === auftragId || String(sx.id) === String(auftragId))){ a = sx; break; }
  }
  if(!a) return;
  if(!a.kommentare) a.kommentare=[];
  var ma = ccAktivMA();
...
function mobMarkKommentareGesehenFuerAuftrag(auftragId, maIdOpt){
  ...
  var maId = maIdOpt != null && String(maIdOpt).trim() !== '' ? String(maIdOpt).trim()
    : (typeof MOB_MA_ID !== 'undefined' && MOB_MA_ID ? String(MOB_MA_ID).trim() : '');
  if(!maId) return;
  var a = null;
  var i, x;
  for(i = 0; i < AUFTRAEGE.length; i++){
    x = AUFTRAEGE[i];
    if(x && (x.id === auftragId || String(x.id) === String(auftragId))){ a = x; break; }
  }
...
function updateGlocke(){
  ...
  if (typeof ccNotifBadgeUpdate === 'function') ccNotifBadgeUpdate();
  if (typeof ccNotifRender === 'function' && typeof CC_NOTIF_OPEN !== 'undefined' && CC_NOTIF_OPEN) ccNotifRender();
...
function auDetailAktion(typ, auId){
  ...
  if(typ==='termin'){
    ...
    ccInternConfirm('Schritt "'+STEP_LABELS[a.step].title+'" als fertig markieren?', function(){
...
/**
 * Nur Mitarbeiter aus MA_DATA (API), die zu den konfigurierten Schritt-Kürzeln passen.
...
function auSchrittVerantwortlicheZeilen(maOptionIds) {
  ...
  for (var i = 0; i < cfgIds.length; i++) {
    var m = typeof maDataFindByWorkflowKey === 'function' ? maDataFindByWorkflowKey(cfgIds[i]) : null;
    if (m) out.push(m);
  }
  return out;
}
...
function maIdGleich(a, b) {
  ...
  if (aM && !bM) { return String(aM.id) === String(b) || String(aM.maId) === String(b) || (aM.k && String(aM.k).toUpperCase() === String(b).toUpperCase()); }
  if (bM && !aM) { return String(bM.id) === String(a) || String(bM.maId) === String(a) || (bM.k && String(bM.k).toUpperCase() === String(a).toUpperCase()); }
  return false;
}
...
function clChecklistenFuerSchritt(auftrag, schritt){
  ...
  vorlagenIds.forEach(function(vid){
    var v = (window.CL_VORLAGEN || []).find(function(x){ return x.id===vid; });
    if(!v || !v.aktiv) return;
...
function clChecklistenFuerAuftrag(auftrag){
  ...
  var gesehen = {};
  vorlagenIds.forEach(function(vid){
    var vorlage = (window.CL_VORLAGEN || []).find(function(v){ return v.id === vid; });
...
function clVorlagenNamenFuerAuftrag(auftrag){
  ...
  return ids.map(function(id){
    var v = (window.CL_VORLAGEN || []).find(function(x){ return x.id === id; });
    return v ? v.name : id;
  });
}
...
function auftragAufgabenErzeugen(auftragId){
  ...
  stepOrder.forEach(function(step){
    ...
    var rawList = internSchrittResolvedUserIds(sch);
    if ((!rawList || !rawList.length) && sch.wer && String(sch.wer).trim() && sch.wer !== '—') {
      var nIdsLegacy = [];
      String(sch.wer)
        .split('+')
      ...
      if (nIdsLegacy.length) rawList = nIdsLegacy;
    }
    var uniqUuids = [];
    var seenUuid = {};
    (rawList || []).forEach(function (raw) {
      var r = raw != null ? String(raw).trim() : '';
      if (!r || r === 'undefined') return;
      var resolved = r;
      if (typeof maIstCockpitUserUuid === 'function' && !maIstCockpitUserUuid(resolved)) {
        resolved =
          typeof maKuerzelOderIdZuUserUuid === 'function' ? String(maKuerzelOderIdZuUserUuid(r) || '').trim() : '';
      }
      if (!resolved || (typeof maIstCockpitUserUuid === 'function' && !maIstCockpitUserUuid(resolved))) {
        if (typeof console !== 'undefined' && console.warn) {
          console.warn('[auftragAufgabenErzeugen] überspringe MA ohne auflösbare User-UUID', {
            auftragId: auftragId,
            step: step,
            token: r,
          });
      ...
      if (seenUuid[resolved]) return;
      seenUuid[resolved] = true;
      uniqUuids.push(resolved);
    });
    ...
    if (step === 'montage') {
      ...
      uniqUuids.forEach(function (rawMaId) {
        ...
        var stepVerantId = (sch && (sch.verantwortlicher || sch.werId || sch.maId))
          ? String(sch.verantwortlicher || sch.werId || sch.maId)
          : null;
        var aufgabeM = {
          id: aufgId,
          auftragId: auftragId,
        ...
        aufgabeM.checkliste = clChecklistenFuerSchritt(a, step);
    ...
    uniqUuids.forEach(function (rawMaId) {
      ...
      bloecke.forEach(function(block, idx){
        ...
        var stepVerantId = (sch && (sch.verantwortlicher || sch.werId || sch.maId)) ? String(sch.verantwortlicher || sch.werId || sch.maId) : null;
        var aufgabe = {
          id:          aufgId,
          auftragId:   auftragId,
          fz:          a.fz    || a.id,
 ...
 */
function ccInternRebuildInternAufgabenFromAuftraege(){
  INTERN_AUFGABEN.length = 0;
  aufgabenNr = 1;
  AUFTRAEGE.forEach(function (a) {
    if (a && a.id != null) {

...
import { renderFusaApiAuftraegeUmzugTableInnerHtml, formatFusaFahrzeugIdsShort } from '../../../shared/ui/auftraege-api-table.js';
import { resolveFusaAuftragUiStatus, fusaAuftragBadgeClassesForBucket } from '../../lib/fusa-auftrag-ui-status.js';
import { buildFusaAuftragWizardShellHtml, bootFusaAuftragWizard } from './fusa-auftrag-wizard.js';
...
function defaultAbnahmeProtokollState() {
  ...
  return {
    ...
    monteur2: '',
    montagezeit: '07:00',
    checklist: FUSA_ABNAHME_CHECKLIST_PUNKTE.map(() => ({ ok: true, mangel: false, bemerkung: '' })),
    bemerkungen: '',
    status: 'ok',
    signatur_monteur: '',
    signatur_werkstatt: '',
...
function normalizeAbnahmeProtokollFromExtra(ex) {
  ...
  if (o.status === 'ok' || o.status === 'mangel' || o.status === 'nein') st.status = o.status;
  if (o.montage_datum != null) st.montage_datum = String(o.montage_datum).slice(0, 10);
  ...
  if (o.aktualisiert_iso != null) st.aktualisiert_iso = String(o.aktualisiert_iso);
...
function fusaAbnahmeUpdateMonteurMetaLine(scope) {
  ...
  const df = d && /^\d{4}-\d{2}-\d{2}$/.test(d) ? d.split('-').reverse().join('.') : d || '—';
...
function buildAbnahmeProtokollMountHtml(auftrag, st) {
  const row = auftrag && typeof auftrag === 'object' ? /** @type {Record<string, unknown>} */ (auftrag) : {};
  const ex = parseFusaExtraFromRow(row);
  const kampId = row.id != null ? String(row.id) : '—';
  const kunde =
    row.kunde_name != null && String(row.kunde_name).trim() !== '' ? String(row.kunde_name).trim() : '—';
  const fzKurz =
  ...
  const lz =
    t0 && t1 ? `${formatDeDatumShort(t0)} – ${formatDeDatumShort(t1)}` : t0 ? formatDeDatumShort(t0) : '—';
  const kz = ex.fahrzeug_kennzeichen != null ? String(ex.fahrzeug_kennzeichen) : '—';
  const fzTyp = ex.fahrzeugtyp != null ? String(ex.fahrzeugtyp) : '—';
  const depot = ex.depot != null ? String(ex.depot) : '—';
  const betr = ex.fahrzeug_betreiber != null ? String(ex.fahrzeug_betreiber) : '—';
  ...
  const md = st.montage_datum && /^\d{4}-\d{2}-\d{2}$/.test(st.montage_datum) ? st.montage_datum : new Date().toISOString().slice(0, 10);
...
function wireAbnahmeSignaturePads(scope, saved, opts) {
  ...
  const sigState = { monteur: { drawing: false, hasData: false }, werkstatt: { drawing: false, hasData: false } };
  const disposes = [];
  ...
  if (saved) {
    if (saved.signatur_monteur) applySavedToCanvas('monteur', saved.signatur_monteur);
    if (saved.signatur_werkstatt) applySavedToCanvas('werkstatt', saved.signatur_werkstatt);
  }
  ...
  for (const who of whoList) {
    ...
    const onUp = e => {
      ...
      if (e && typeof e === 'object' && 'pointerId' in e) {
        try {
          canvas.releasePointerCapture(/** @type {PointerEvent} */ (e).pointerId);
        } catch {
          /* ignore */
        }
      }
  ...
  const getDataUrl = who => {
    const canvas = scope.querySelector(`canvas[data-fusa-sig-canvas="${who}"]`);
    if (!(canvas instanceof HTMLCanvasElement) || !sigState[who]?.hasData) return null;
...
function printAbnahmeProtokollFromDom(abnahmeRoot, auftragId) {
  const content = abnahmeRoot.querySelector('[data-fusa-auf-abnahme-print-root]');
  if (!(content instanceof HTMLElement)) return;
  const sigApi = /** @type {any} */ (abnahmeRoot)._fusaSigApi;
  const getUrl = w => (sigApi && typeof sigApi.getDataUrl === 'function' ? sigApi.getDataUrl(w) : null);
  const sigM = getUrl('monteur');
  const sigW = getUrl('werkstatt');
  if (!sigM && !sigW) {
    if (!window.confirm('Noch keine Unterschriften vorhanden. Trotzdem drucken?')) return;
  }
  ...
  for (const who of ['monteur', 'werkstatt']) {
    ...
    if (canvas && sigData) {
      ...
      img.src = sigData;
      img.style.cssText = 'display:block;width:100%;height:110px;object-fit:contain;';
  ...
  clone.querySelectorAll('input[type="text"],input[type="time"],input[type="date"]').forEach(inp => {
  ...
  if (!win) {
    window.alert('Pop-up blockiert — Druckfenster konnte nicht geöffnet werden.');
    return;
  }
  const sc = '<scr' + 'ipt>setTimeout(function(){window.print();},500);</scr' + 'ipt>';
  win.document.write(
...
function collectAbnahmeProtokollStateFromDom(scope, canBearbeiten, sigApi) {
  ...
  for (const r of radios) {
    if (r instanceof HTMLInputElement && r.checked && (r.value === 'ok' || r.value === 'mangel' || r.value === 'nein')) {
  ...
  const montage_datum = md instanceof HTMLInputElement ? md.value.trim().slice(0, 10) : '';
  ...
  const name_werkstatt = wnEl instanceof HTMLInputElement ? wnEl.value.trim() : '';
...
const FUSA_AUF_DP_TYP_ROWS = [
  { key: 'vertrag', icon: '📋', label: 'Vertrag', color: '#D4500A', bg: '#E6F1FB' },
  { key: 'freigabe', icon: '✅', label: 'Freigabe', color: '#0F6E56', bg: '#E1F5EE' },
  { key: 'layout', icon: '🎨', label: 'Layout/Druckdatei', color: '#854F0B', bg: '#FAEEDA' },
  { key: 'rechnung', icon: '💶', label: 'Rechnung', color: '#5B35B0', bg: '#EDE9FE' },
  { key: 'foto', icon: '📷', label: 'Montagefoto', color: '#0A5A8A', bg: '#E6F1FB' },
  { key: 'sonstiges', icon: '📎', label: 'Sonstiges', color: '#546E7A', bg: '#ECEFF1' },
];
...
function openFusaAufDpFahrzeugFromRow(aufRow, bodyEl, closeDetail) {
  ...
  if (!fzId && empty) {
    if (bodyEl) showFusaAufDetailTransient(bodyEl, '⚠ Kein Fahrzeug zugeordnet');
    return;
  }
  if (fzId) {
    closeDetail();
...
function readFusaAufDpSelectedTypFromGrid(bodyEl) {
  ...
  for (const el of grid.querySelectorAll('[data-fusa-auf-dp-typ]')) {
    ...
    if (el.style.opacity !== '0.4') typ = el.getAttribute('data-typ') || 'sonstiges';
  }
  return typ;
}
...
function showFusaAufDpTypwahlAfterFile(bodyEl, fileNameShort) {
  ...
  if (save instanceof HTMLElement) save.textContent = short ? `💾 Speichern: ${short}` : 'Speichern';
...
function buildFusaAufDetailDocTypwahlBlockHtml(canBearbeiten) {
  ...
  const cells = FUSA_AUF_DP_TYP_ROWS.map(
    t => `<div role="button" tabindex="0" data-fusa-auf-dp-typ data-typ="${esc(t.key)}"
 ...
 */
async function buildFusaAuftragDetailBodyHtml(row, projectNameById, canBearbeiten = false) {
  void projectNameById;
  const kundeRow = { ...(row && typeof row === 'object' ? row : {}), firma_id: row.fusa_kunde_id ?? row.firma_id };
  const apiKunde = row.kunde_name != null && String(row.kunde_name).trim() !== '' ? String(row.kunde_name).trim() : '';
  ...
  const ex = parseFusaExtraFromRow(row);
  const paketShow = ex.paket != null && String(ex.paket).trim() !== '' ? String(ex.paket).trim() : '—';

  const lzMon =
    ex.laufzeit_monate != null && String(ex.laufzeit_monate).trim() !== '' && Number.isFinite(Number(ex.laufzeit_monate))
  ...
  const pill = (text) => {
    if (!text || text === '—') return `<span class="fusa-auf-detail-kv__val fusa-auf-detail-kv__val--muted">—</span>`;
  ...
  const vsN = av.naechste_periode != null ? String(av.naechste_periode).trim() : '';
  const vsF = av.folgeperiode != null ? String(av.folgeperiode).trim() : '';

  const docs = Array.isArray(ex.dokumente_meta) ? /** @type {unknown[]} */ (ex.dokumente_meta) : [];
  ...
  if (nDocs > 0) {
    docListHtml = `<ul class="fusa-auf-detail-doclist">${docs
      .map((d, idx) => {
        ...
        const metaBits = [fusaAufDpTypLabel(typ), datum, szLab].filter(x => x && x !== '—');
...
export async function renderFusaAuftraegeViewHtml() {
  ...
  const canBearbeiten = myRight(rightsBundle, 'fusa', 'auftraege', 'bearbeiten');
  ...
  let filtered = pid ? auftraegeAll.filter(a => a && String(a.project_id || '') === pid) : auftraegeAll;
  if (!filtered.length && auftraegeAll.length > 0 && projects.length > 0) {
    const idsFromApi = [
      ...new Set(
        auftraegeAll
          .map(a => (a && a.project_id != null ? String(a.project_id).trim() : ''))
          .filter(Boolean),
      ),
    ];
    const fallbackPid = idsFromApi.find(pjid => projects.some(p => String(p.id) === pjid));
    if (fallbackPid && fallbackPid !== pid) {
      await loadFusaProjectContext(fallbackPid);
      ctx = getFusaAppProject();
      pid = ctx && ctx.id ? String(ctx.id) : '';
      filtered = pid ? auftraegeAll.filter(a => a && String(a.project_id || '') === pid) : auftraegeAll;
    }
  }

  const nameById = Object.fromEntries(
    projects.map(p => [String(p.id), p.name != null ? String(p.name) : String(p.id)]),
  );
...
function applyFusaAuftragRowFilters(root) {
  ...
  for (const tr of root.querySelectorAll('tbody tr[data-fusa-auf-payload]')) {
    ...
    let hay = '';
    try {
      hay = decodeURIComponent(tr.getAttribute('data-fusa-auf-search-hay') || '');
    } catch {
      hay = '';
    }
...
export function attachFusaAuftraegeViewHandlers(mount, onReload) {
  ...
  const root =
    mount instanceof Element
      ? mount.querySelector('.fusa-umz-auf-scope[data-ccw-ro="fusa-auftraege"]') ||
  ...
  async function openNeu() {
    ...
    if (wRoot instanceof HTMLElement) {
      const preFz =
        CCState.get('fusaAuftragNeuFahrzeugId') != null && String(CCState.get('fusaAuftragNeuFahrzeugId')).trim()
          ? String(CCState.get('fusaAuftragNeuFahrzeugId')).trim()
          : '';
      const preNotiz =
        CCState.get('fusaAuftragNeuInternNotiz') != null && String(CCState.get('fusaAuftragNeuInternNotiz')).trim()
  ...
  async function openWizardForEdit(auftragId) {
    ...
    try {
      const d = await apiFetch(`${API_ROUTES.fusa.auftraege}/${encodeURIComponent(id)}`);
      const row = d && typeof d === 'object' && /** @type {any} */ (d).auftrag ? /** @type {any} */ (d).auftrag : null;
      const pid = row && row.project_id != null ? String(row.project_id).trim() : '';
  ...
  const canBearbeitenFromDom = root.getAttribute('data-fusa-can-bearbeiten') === '1';
  ...
  function closeAbnahmeProtokoll() {
    ...
    const mount = abnahmeModal.querySelector('[data-fusa-auf-abnahme-print-mount]');
  ...
  async function openAbnahmeProtokollFromDetail(auftragId) {
    ...
    if (mount instanceof HTMLElement) mount.innerHTML = '<p class="ckp-mock-note" style="padding:16px 20px;">Lade Abnahmeprotokoll…</p>';
    ...
    try {
      const d = await apiFetch(`${API_ROUTES.fusa.auftraege}/${encodeURIComponent(aid)}`);
      const auf =
        d && typeof d === 'object' && /** @type {any} */ (d).auftrag != null ? /** @type {any} */ (d).auftrag : null;
      if (!auf || typeof auf !== 'object') throw new Error('Auftrag nicht gefunden.');
      ...
      if (mount instanceof HTMLElement) {
        ...
        if (!canBearbeitenFromDom) {
          for (const el of mount.querySelectorAll('input, textarea, button[data-fusa-sig-clear]')) {
        ...
        const sig = wireAbnahmeSignaturePads(mount, st, { readOnly: !canBearbeitenFromDom });
        if (!canBearbeitenFromDom) {
      ...
      const kunde =
        rowR.kunde_name != null && String(rowR.kunde_name).trim() !== '' ? String(rowR.kunde_name).trim() : '—';
      const fzKurz =
        rowR.fahrzeug_kurztext != null && String(rowR.fahrzeug_kurztext).trim() !== ''
          ? String(rowR.fahrzeug_kurztext).trim()
          : '';
      const fzLine = fzKurz || formatFusaFahrzeugIdsShort(rowR.fusa_fahrzeug_ids);
    ...
    try {
      window.dispatchEvent(
        new CustomEvent('fusa:auftrag:abnahme', { detail: { auftragId: aid, phase: 'open' }, bubbles: true }),
      );
    } catch {
      /* ignore */
    }
  ...
  async function saveAbnahmeProtokoll() {
    const aid = abnahmeModal instanceof HTMLElement ? abnahmeModal.getAttribute('data-fusa-abnahme-auftrag-id') || '' : '';
    const mount =
      abnahmeModal instanceof HTMLElement ? abnahmeModal.querySelector('[data-fusa-auf-abnahme-print-mount]') : null;
    const sigApi = /** @type {any} */ (abnahmeModal instanceof HTMLElement ? abnahmeModal._fusaSigApi : null);
    ...
    if (!collected) return;
    const sM =
      (sigApi && typeof sigApi.getDataUrl === 'function' ? sigApi.getDataUrl('monteur') : null) ||
      (abnahmeSigCleared.monteur ? '' : abnahmeBaseline?.signatur_monteur || '');
    const sW =
      (sigApi && typeof sigApi.getDataUrl === 'function' ? sigApi.getDataUrl('werkstatt') : null) ||
      (abnahmeSigCleared.werkstatt ? '' : abnahmeBaseline?.signatur_werkstatt || '');
    const nowIso = new Date().toISOString();
    const payload = {
      ...collected,
    ...
    try {
      await apiFetch(`${API_ROUTES.fusa.auftraege}/${encodeURIComponent(aid)}`, {
        method: 'PATCH',
        body: {
          fusa_extra_json: {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent('fusa:auftrag:abnahme', { detail: { auftragId: aid, phase: 'saved' }, bubbles: true }),
        );
      } catch {
        /* ignore */
      }
  ...
  root.addEventListener('click', async ev => {
    ...
    if (modal instanceof HTMLElement && modal.classList.contains('open') && t.closest && t.closest('[data-fusa-auf-detail-modal]')) {
      return;
    }
    if (
    ...
    if (!raw || !(modal instanceof HTMLElement) || !(bodyEl instanceof HTMLElement)) return;
    let row;
    try {
    ...
    modal.setAttribute('data-fusa-detail-auftrag-id', row.id != null ? String(row.id) : '');
    const headTit = root.querySelector('[data-fusa-auf-detail-title]');
    if (headTit) {
      const idShow = row.id != null ? String(row.id) : '';
      headTit.textContent = idShow || (row.title != null ? String(row.title).trim() : '') || 'Auftrag';
    }
    const idStr = row.id != null ? String(row.id).trim() : '';
    try {
      let detailRow = row;
      if (idStr) {
        const d = await apiFetch(`${API_ROUTES.fusa.auftraege}/${encodeURIComponent(idStr)}`);
        const auf =
          d && typeof d === 'object' && /** @type {Record<string, unknown>} */ (d).auftrag != null
      ...
      bodyEl.innerHTML = await buildFusaAuftragDetailBodyHtml(detailRow, nameById, canBearbeitenFromDom);
    } catch (e) {
      try {
        const warn =
          idStr ?
          ...
          : '';
        bodyEl.innerHTML = warn + (await buildFusaAuftragDetailBodyHtml(row, nameById, canBearbeitenFromDom));
      } catch {
        bodyEl.innerHTML = '<p class="ckp-api-error" role="alert">Detail konnte nicht aufgebaut werden.</p>';
      }
    }
  });
  ...
  if (modal instanceof HTMLElement) {
    ...
    async function persistDokumenteAndRefresh(nextDocs) {
      ...
      const patchRes = await apiFetch(`${API_ROUTES.fusa.auftraege}/${encodeURIComponent(id)}`, {
        method: 'PATCH',
        body: { fusa_extra_json: { dokumente_meta: nextDocs } },
      });
      const auf =
        patchRes && typeof patchRes === 'object' && /** @type {any} */ (patchRes).auftrag != null
          ? /** @type {object} */ (/** @type {any} */ (patchRes).auftrag)
          : null;
      if (!auf) throw new Error('Speichern fehlgeschlagen.');
      bodyEl.innerHTML = await buildFusaAuftragDetailBodyHtml(auf, nameById, canBearbeitenFromDom);
    }
    ...
    function triggerDocFilePicker() {
      ...
      else showFusaAufDetailTransient(bodyEl, '⚠ Bitte Auftrag öffnen um Dateien hochzuladen');
    }
    ...
    modal.addEventListener('change', ev => {
      const t = ev.target;
      if (!(t instanceof HTMLInputElement) || !t.matches('[data-fusa-auf-dp-file]')) return;
      if (!(bodyEl instanceof HTMLElement) || !canBearbeitenFromDom) return;
      const files = t.files;
    ...
    modal.addEventListener('click', ev => {
      ...
      if (delBtn instanceof HTMLElement) {
        ...
        void (async () => {
          ...
          if (!id || !(bodyEl instanceof HTMLElement)) return;
          try {
            const d0 = await apiFetch(`${API_ROUTES.fusa.auftraege}/${encodeURIComponent(id)}`);
            const auf0 =
              d0 && typeof d0 === 'object' && /** @type {any} */ (d0).auftrag != null
                ? /** @type {object} */ (/** @type {any} */ (d0).auftrag)
                : null;
            if (!auf0) throw new Error('Auftrag nicht gefunden.');
            const ex0 = parseFusaExtraFromRow(auf0);
            const cur = Array.isArray(ex0.dokumente_meta) ? [...ex0.dokumente_meta] : [];
            if (idx < 0 || idx >= cur.length) return;
            const removed = cur[idx];
            const n =
              removed && typeof removed === 'object' && /** @type {any} */ (removed).name != null
      ...
      if (el.closest('[data-fusa-auf-dp-save]')) {
        ...
        void (async () => {
          ...
          try {
            ...
            await persistDokumenteAndRefresh(cur);
            dpPendingFile = null;
            const shortName = f.name.length > 30 ? `${f.name.slice(0, 30)}…` : f.name;
            showFusaAufDetailTransient(bodyEl, `✓ ${fusaAufDpTypLabel(typ)} gespeichert: ${shortName}`);
          } catch (e) {
            window.alert(formatApiErrorForUi(e));
          }
        })();
      ...
      if (actBtn instanceof HTMLElement) {
        ...
        if (k === 'rechnung') {
          if (bodyEl instanceof HTMLElement) showFusaAufDetailTransient(bodyEl, 'Rechnung wird erstellt…');
          return;
        }
        return;
      }
  ...
  if (abnahmeModal instanceof HTMLElement) {
    ...
    abnahmeModal.addEventListener('input', ev => {
      ...
      const wrap = t.closest('[data-fusa-auf-abnahme-print-mount]');
      if (!(wrap instanceof HTMLElement)) return;
      if (t.matches('[data-fusa-ap-monteur1], [data-fusa-ap-monteur2], [data-fusa-ap-datum]')) {
    ...
    abnahmeModal.addEventListener('click', ev => {
      ...
      if (el.closest('[data-fusa-auf-abnahme-print]')) {
        ev.preventDefault();
        const aid = abnahmeModal.getAttribute('data-fusa-abnahme-auftrag-id') || '';

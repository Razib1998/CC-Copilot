...
function effektivePruefSlot(w, vgl) {
  ...
  // DPI-, Farbraum- und Schriften-Zeilen ignorieren — Druckrisiko wird in MesseFlow
  // per Confirm + wand.druckFreigabe dokumentiert, blockiert den Ampel-Slot nicht hart.
  const zeilen = (pr.pruefung || []).filter(z => {
    const t = String(z.titel || '').toLowerCase();
    return !t.includes('dpi')
      && !t.includes('auflösung')
      && !t.includes('farbraum')
      && !t.includes('schriften');
  });
...
function mfNextCcInternId() {
  ...
  const bump = (id) => {
    if (!id) return;
    const m = String(id).match(/^CC-(\d{4})-(\d+)$/);
    if (m && parseInt(m[1], 10) === year) max = Math.max(max, parseInt(m[2], 10));
  };
  (MesseFlowState.projects || []).forEach(p => bump(p.ccinternAuftrag?.id));
  if (typeof window !== 'undefined' && Array.isArray(window.AUFTRAEGE)) {
    window.AUFTRAEGE.forEach(a => bump(a && a.id));
  }
...
function mfLieferterminKanbanDe(isoYmd) {
  const s = String(isoYmd || '').slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) return new Date().toLocaleDateString('de-DE');
  try {
...
function mfBuildCcInternKanbanRow(auftrag, messeflowProjektId) {
  const p = typeof getP === 'function' ? getP(messeflowProjektId) : null;
  const lieferRaw =
    auftrag.liefertermin
    || (p && (p.deadline || (p.auftragsInfo && p.auftragsInfo.liefertermin)))
    || null;
  const lieferIso = lieferRaw ? String(lieferRaw).slice(0, 10) : new Date().toISOString().slice(0, 10);
  const schritte = {
    grafik: mfMinimalProdSchritt(),
    druck: mfMinimalProdSchritt(),
    laminat: mfMinimalProdSchritt(),
    montage: Object.assign(mfMinimalProdSchritt(), { typ: 'multi', fotosErforderlich: true }),
    doku: Object.assign(mfMinimalProdSchritt(), { typ: 'single', fotosErforderlich: true }),
    abgeschlossen: { wer: null, maId: null, dauer: 0, fertig: false, zeit: null },
  };
  return {
    id: auftrag.id,
    kunde: auftrag.kunde || '–',
    fz: auftrag.veranstaltung || auftrag.bezeichnung || 'MesseFlow',
    paket: auftrag.bezeichnung || 'MesseFlow-Übergabe',
    terminDatum: lieferIso,
    montageDatum: null,
    montageZeit: null,
    liefertermin: mfLieferterminKanbanDe(lieferIso),
    depot: '',
    step: 'grafik',
    urgent: false,
    rechnung: 'offen',
    ...
    schritte,
    prod: {
      planung: { notiz: `Quelle: MesseFlow · ${messeflowProjektId}` },
      produktion: { bestaetigt: false },
      template: {},
      dateien: [],
    },
    sourceSystem: auftrag.sourceSystem || 'messeflow',
    sourceId: auftrag.sourceId || messeflowProjektId,
    messeflowProjektId,
    messeflowCcInternAuftrag: auftrag,
    kommentare: [],
  };
}

function mfSyncCcInternProductionAuftrag(messeflowProjektId, auftrag) {
  if (typeof window === 'undefined' || !Array.isArray(window.AUFTRAEGE) || !auftrag) return;
  const sourceId = String(auftrag.sourceId || messeflowProjektId || '');
  if (!sourceId) return;
  if (window.AUFTRAEGE.some(function (a) {
    return a && a.sourceSystem === 'messeflow' && String(a.sourceId || '') === sourceId;
  })) return;
  if (window.AUFTRAEGE.some(function (a) { return a && a.id === auftrag.id; })) return;
  window.AUFTRAEGE.push(mfBuildCcInternKanbanRow(auftrag, messeflowProjektId));
  if (typeof window.saveAuftraege === 'function') window.saveAuftraege();
}
...
function mfUebergabePruefen(projektId) {
  ...
  mfSyncCcInternProductionAuftrag(projektId, auftrag);
...
/** Cockpit-Embed: gleiche Origin + Bearer via `window.__mfPruefFetch`; Standalone: normales `fetch`. */
function mfPruefHttp(url, init) {
  if (typeof window !== 'undefined' && typeof window.__mfPruefFetch === 'function') {
    return window.__mfPruefFetch(url, init || {});
  }
  return fetch(url, init);
}
if (typeof window !== 'undefined') {
  window.mfPruefHttp = mfPruefHttp;
}
...
async function checkServerStatus(){
  ...
  try {
    const res  = await mfPruefHttp(`${CALDERA_SERVER}/status`, { signal: AbortSignal.timeout(3000) });
    const data = await res.json();
...
async function exportWandZuCaldera(pid, wid){
  ...
  try {
    // 1. Ordner anlegen
    const ordnerRes = await mfPruefHttp(`${CALDERA_SERVER}/export/ordner`, {
      method: 'POST',
    ...
    const dateiRes = await mfPruefHttp(`${CALDERA_SERVER}/export/datei`, {
      method: 'POST',
      body: form,
    });

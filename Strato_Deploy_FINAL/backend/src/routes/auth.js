...
export function createAuthRouter(store) {
  ...
  const authMeChain = [maybeAttachDevProvisionAuth(store), requireAuth, attachAccessProfile(store)];
  ...
  router.get('/my-rights', ...authMeChain, async (req, res, next) => {
    try {
  ...
  router.get('/me', ...authMeChain, async (req, res, next) => {
    try {

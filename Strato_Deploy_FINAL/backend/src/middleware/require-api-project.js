...
export function requireApiProjectContext(store) {
  return async (req, res, next) => {
    try {
      ...
      if (!access) {
        const hasCockpitProjekte =
          profile &&
          profile.hasModule('cockpit') &&
          profile.has('cockpit', 'projekte', 'sehen');
        const appOnlyMitarbeiter =
          profile &&
          profile.hasModule('ccintern') &&
          profile.has('ccintern', 'mitarbeiterapp', 'sehen') &&
          !hasCockpitProjekte;
        if (appOnlyMitarbeiter) {
          req.apiProjectId = pid;
          return next();
        }

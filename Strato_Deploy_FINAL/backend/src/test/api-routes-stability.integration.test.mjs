...
test('backend/src/routes: Syntax-Check (node --check)', () => {
  ...
  const nestedDashboard = [
    ...
    'ccintern/mobile.js',
    'ccintern/checklisten-zuordnung.js',
    'crm/index.js',
  ].map((f) => path.join(routesDir, f));
...
test('backend/src/routes: Module importieren (kein Load-Time ReferenceError)', async () => {
  const specs = [
    ...
    ['../routes/ccintern/checklisten-zuordnung.js', 'createCcInternChecklistenZuordnungRouter'],
...
test('GET /api/v1/users, auftraege, firmen, kunden, fusa/fahrzeuge — JSON 200', async (t) => {
  ...
  const cases = [
    ...
    {
      name: 'GET /api/v1/ccintern/checklisten-zuordnung (leer)',
      path: `/api/v1/ccintern/checklisten-zuordnung?firma_id=${firmaId}`,
      assert: (res, body) => {
        assert.equal(res.status, 200, JSON.stringify(body));
        assert.equal(body?.success, true);
        assert.ok(Array.isArray(body?.data?.items), 'body.data.items');
        assert.equal(body.data.items.length, 0, 'ohne DB-Zeilen leer');
      },
    },
    {
  ...
  await t.test('GET /api/v1/ccintern/checklisten-zuordnung — Fremdfirma + produkt_id Filter', async () => {
    const otherFid = randomUUID();
    await store.insertFirma({
      id: otherFid,
      name: `Other Firma ${otherFid.slice(0, 6)}`,
      kundennummer: '',
      altnummer: '',
      typ: 'kunde',
      intern_extern: 'extern',
    ...
    const clOwn = randomUUID();
    const clOther = randomUUID();
    await store.insertCheckliste({
      id: clOwn,
      titel: 'Zuordnung Test Own',
      firma_id: firmaId,
      auftrag_id: null,
      erstellt_von: null,
    });
    await store.insertCheckliste({
      id: clOther,
      titel: 'Zuordnung Test Other',
      firma_id: otherFid,
      auftrag_id: null,
      erstellt_von: null,
    });
    const zOwn = randomUUID();
    const zOther = randomUUID();
    /** @type {{ db?: { run: (sql: string, params?: unknown[]) => void } }} */
    const raw = /** @type {any} */ (store);
    assert.ok(raw.db && typeof raw.db.run === 'function', 'SQLite-Store mit db.run');
    raw.db.run(
      `INSERT INTO ccintern_checklisten_zuordnung (id, firma_id, produkt_id, schritt, checkliste_id, sortierung, aktiv, created_at, updated_at) VALUES (?,?,?,?,?,?,1,datetime('now'),NULL)`,
      [zOwn, firmaId, 'pkw_teil', 'grafik', clOwn, 0],
    );
    raw.db.run(
      `INSERT INTO ccintern_checklisten_zuordnung (id, firma_id, produkt_id, schritt, checkliste_id, sortierung, aktiv, created_at, updated_at) VALUES (?,?,?,?,?,?,1,datetime('now'),NULL)`,
      [zOther, otherFid, 'pkw_teil', 'grafik', clOther, 0],
    );
    await store.persist();

    const { res: r1, body: b1 } = await apiGet(`/api/v1/ccintern/checklisten-zuordnung?firma_id=${firmaId}`);
    assert.equal(r1.status, 200, JSON.stringify(b1));
    assert.equal(b1?.success, true);
    assert.equal(b1.data.items.length, 1, 'nur eigene Firma');
    assert.equal(b1.data.items[0].id, zOwn);
    assert.equal(b1.data.items[0].firma_id, firmaId);

    const { res: r2, body: b2 } = await apiGet(
      `/api/v1/ccintern/checklisten-zuordnung?firma_id=${firmaId}&produkt_id=pkw_teil`,
    );
    assert.equal(r2.status, 200);
    assert.equal(b2.data.items.length, 1);

    const { res: r3, body: b3 } = await apiGet(
      `/api/v1/ccintern/checklisten-zuordnung?firma_id=${firmaId}&produkt_id=bus_voll`,
    );
    assert.equal(r3.status, 200);
    assert.equal(b3.data.items.length, 0);
  });

  await t.test('POST/PATCH/DELETE /api/v1/ccintern/checklisten-zuordnung', async () => {
    const otherFid = randomUUID();
    await store.insertFirma({
      id: otherFid,
      name: `Other Z ${otherFid.slice(0, 6)}`,
      kundennummer: '',
      altnummer: '',
      typ: 'kunde',
      intern_extern: 'extern',
    ...
    const postUrl = new URL('/api/v1/ccintern/checklisten-zuordnung', serverOrigin).toString();

    const { res: rBadStep } = await httpJson('POST', postUrl, {
      headers: zuHdr,
      jsonBody: { produkt_id: 'pkw_teil', schritt: 'invalid_step', checkliste_id: clOwn },
    });
    assert.equal(rBadStep.status, 400);

    const { res: rBadCl } = await httpJson('POST', postUrl, {
      headers: zuHdr,
      jsonBody: { produkt_id: 'pkw_teil', schritt: 'grafik', checkliste_id: clOther },
    });
    assert.equal(rBadCl.status, 400);

    const { res: rOk, body: bOk } = await httpJson('POST', postUrl, {
      headers: zuHdr,
      jsonBody: { produkt_id: 'pkw_teil', schritt: 'laminat', checkliste_id: clOwn, sortierung: 2 },
    });
    assert.equal(rOk.status, 201, JSON.stringify(bOk));
    assert.equal(bOk?.success, true);
    const zid = bOk?.data?.item?.id;
    assert.ok(zid);

    const { res: rGet, body: bGet } = await apiGet(`/api/v1/ccintern/checklisten-zuordnung?firma_id=${firmaId}`);
    assert.equal(rGet.status, 200);
    const hit = bGet.data.items.find((/** @type {any} */ x) => x.id === zid);
    assert.ok(hit, 'POST-Zuordnung über GET sichtbar');
    assert.equal(hit.schritt, 'laminat');

    const patchUrl = new URL(`/api/v1/ccintern/checklisten-zuordnung/${encodeURIComponent(zid)}`, serverOrigin).toString();
    const { res: rPatch, body: bPatch } = await httpJson('PATCH', patchUrl, {
      headers: zuHdr,
      jsonBody: { sortierung: 7 },
    });
    assert.equal(rPatch.status, 200, JSON.stringify(bPatch));
    assert.equal(bPatch?.data?.item?.sortierung, 7);

    const zForeign = randomUUID();
    const raw = /** @type {any} */ (store);
    assert.ok(raw.db && typeof raw.db.run === 'function', 'SQLite-Store mit db.run');
    raw.db.run(
      `INSERT INTO ccintern_checklisten_zuordnung (id, firma_id, produkt_id, schritt, checkliste_id, sortierung, aktiv, created_at, updated_at) VALUES (?,?,?,?,?,?,1,datetime('now'),NULL)`,
      [zForeign, otherFid, 'pkw_teil', 'druck', clOther, 0],
    );
    await store.persist();

    const patchForeignUrl = new URL(
      `/api/v1/ccintern/checklisten-zuordnung/${encodeURIComponent(zForeign)}`,
      serverOrigin,
    ).toString();
    const { res: rPatch404 } = await httpJson('PATCH', patchForeignUrl, {
      headers: zuHdr,
      jsonBody: { sortierung: 1 },
    });
    assert.equal(rPatch404.status, 404);

    const { res: rDel404 } = await httpJson('DELETE', patchForeignUrl, { headers: zuHdr });
    assert.equal(rDel404.status, 404);

    const delOwnUrl = new URL(`/api/v1/ccintern/checklisten-zuordnung/${encodeURIComponent(zid)}`, serverOrigin).toString();
    const { res: rDelOk, body: bDelOk } = await httpJson('DELETE', delOwnUrl, { headers: zuHdr });
    assert.equal(rDelOk.status, 200, JSON.stringify(bDelOk));
    assert.equal(bDelOk?.success, true);
    assert.equal(bDelOk?.data?.deleted, true);
  });
});

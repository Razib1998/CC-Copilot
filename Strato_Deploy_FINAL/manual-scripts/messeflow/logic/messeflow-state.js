...
function canSeeProject(user, projekt) {
  // Cockpit-Embed: Sicht über ccintern.messeflow.sehen (keine MF-Mitgliedschaft als Master).
  if (
    typeof window !== 'undefined'
    && window.__MF_COCKPIT_EMBED === true
    && typeof window.mfCockpitMesseflowRight === 'function'
    && projekt
  ) {
    try {
      if (window.mfCockpitMesseflowRight('sehen')) return true;
      return false;
    } catch {
      /* Fallback: klassische Regeln */
    }
  }
...
function getVisibleProjects(userId) {
  if (
    ...
    try {
      if (window.mfCockpitMesseflowRight('sehen')) {
...
/** @returns {typeof ALLE_RECHTE_AUS | null} — nur Cockpit-Embed; sonst null (MF-Matrix). */
function mfCockpitEmbedProjRechteBundle() {
  if (
    typeof window === 'undefined'
    || window.__MF_COCKPIT_EMBED !== true
    || typeof window.mfCockpitMesseflowRight !== 'function'
  ...
  try {
    const mf = window.mfCockpitMesseflowRight;
    if (!mf('sehen')) {
      return { ...ALLE_RECHTE_AUS };
    }
    const bearb = !!mf('bearbeiten');
    // Backend löscht MesseFlow-Projekte derzeit mit „bearbeiten“ — UI folgt beiden Flags.
    const loeschenEff = !!mf('loeschen') || bearb;
    return {
      sehen: true,
      upload: !!mf('upload'),
      freigabe: !!mf('freigeben') || bearb,
      angebote: bearb,
      preise: !!mf('preiseSehen'),
      kommentieren: bearb,
      loeschen: loeschenEff,
      exportieren: !!mf('export'),
      einladen: !!mf('erstellen'),
    };
  } catch {
    return null;
  }
...
function getProjRechte(userId, projektId) {
  const user = USERS.find(u => u.id === userId);
  if (user && isProjGesperrt(projektId, userId)) return { ...ALLE_RECHTE_AUS };

  const cockpitEmbed = mfCockpitEmbedProjRechteBundle();
  if (!user) {
    if (cockpitEmbed) return cockpitEmbed;
    return {
      sehen: false,
      upload: false,
      freigabe: false,
...
function kannDateiFreigeben(userId) {
  if (
    ...
    try {
      const mf = window.mfCockpitMesseflowRight;
      return !!(mf('freigeben') || mf('bearbeiten'));
    } catch {
      return false;
    }
  }
...
function kannCalderaSenden(userId) {
  ...
  const user = USERS.find(u => u.id === userId);
  if (!user || user.aktiv === false || isUserGesperrt(user)) return false;
  return user.rolle === 'cc_intern' || user.rolle === 'admin';
}
...
function setDateiGeliefert(projectId, wandId, menge) {
  ...
  if (!user || isUserGesperrt(user)) return false;
  if (!(user.rolle === 'produktion' || user.rolle === 'cc_intern')) return false;
  const p = state.projects.find(x => x.id === projectId);
  const w = p?.waende?.find(x => x.id === wandId);
  const f = getAktuelleDatei(w);

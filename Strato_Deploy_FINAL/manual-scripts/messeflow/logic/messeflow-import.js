...
function importExcel(event, pid){
  ...
  if (typeof window.mfMayImportMesseflowExcel === 'function' && !window.mfMayImportMesseflowExcel()) {
    const inCockpit = typeof window !== 'undefined' && window.__MF_COCKPIT_EMBED;
    toast(
      'Keine Berechtigung',
      inCockpit
        ? 'Erforderlich: ccintern.messeflow.upload, bearbeiten oder erstellen (GET /auth/my-rights).'
        : 'Nur Administrator, CC Intern oder Zwischenhändler können Projekte per Excel anlegen.',
      'ty',
    );
    return;
  }

  if (typeof XLSX === 'undefined' || !XLSX?.utils || typeof XLSX.read !== 'function') {
    toast('Fehler', 'Excel-Library (XLSX) ist nicht geladen. Bitte Seite neu laden.', 'ty');
    return;
  }
    ...
    try {
      ...
      for(let i=0; i<allRows.length; i++){
        ...
        const hasMotiv = row.some(cell => {
          const t = mfNormExcelToken(cell);
          return t === 'motiv' || t.startsWith('motiv ');
        });
        if(hasMotiv){ headerRowIdx = i; break; }
      }

      // Fallback: Kopfzeile über Spaltenmuster erkennen, auch wenn "Motiv" abweicht.
      if (headerRowIdx === -1) {
        for (let i = 0; i < allRows.length; i++) {
          const row = allRows[i] || [];
          const norm = row.map(c => mfNormExcelToken(c)).filter(Boolean);
          const hasBreite = norm.some(h => h.includes('breite') || h.includes('width') || h.includes(' druck b') || h === 'b mm');
          const hasHoehe = norm.some(h => h.includes('hohe') || h.includes('height') || h.includes(' druck h') || h === 'h mm');
          const hasPos = norm.some(h => h.includes('motiv') || h.includes('flache') || h.includes('bezeichnung') || h === 'pos');
          if (hasPos && hasBreite && hasHoehe) {
            headerRowIdx = i;
            break;
          }
        }
      ...
      const headerNorm = allRows[headerRowIdx].map(c => mfNormExcelToken(c));

      function findColIdx(candidates){
        const candNorm = candidates.map(c => mfNormExcelToken(c)).filter(Boolean);
        return headerNorm.findIndex(h =>
          h && candNorm.some(c => h === c || h.includes(c))
        );
      }
      ...
      if (!flaechenImport.length) {
        openModal('Keine importierbaren Flächen', `
          <p style="font-size:13px;color:var(--muted);margin-bottom:10px;">
            Die Datei wurde gelesen, aber es konnten keine gültigen Flächenzeilen erkannt werden.
          </p>
          <p style="font-size:12px;color:var(--muted);margin-bottom:10px;">
            Erwartet pro Zeile: Motiv + Breite + Höhe (numerisch, größer 0).
          </p>
          <div class="ma" style="margin-top:14px;"><button class="btn ghost sm" onclick="closeModal()">Schließen</button></div>
        `);
        return;
      ...
      if (existing) {
        ...
        if (typeof window.__mfSaveMesseflowWorkspace === 'function') {
          window.__mfSaveMesseflowWorkspace().catch((err) => {
            console.error('[MF] messeflow-workspace PUT:', err);
            if (typeof toast === 'function') {
              toast(
                'Hinweis',
                `Import lokal gespeichert; Server-Übernahme fehlgeschlagen: ${err instanceof Error ? err.message : String(err)}`,
                'ty',
              );
            }
          });

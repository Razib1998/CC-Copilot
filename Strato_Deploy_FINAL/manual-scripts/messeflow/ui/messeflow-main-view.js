// ═══════════════════════════════════════════════════════════════════════════════
// MESSEFLOW MAIN VIEW  ←  Quelle: Messeflow/DEV/js/messeflow-app.js
// Ziel: messeflow-main-view.js (ui/)
//
// Enthält:
//   • App-Initialisierung          messeflowNormalBoot()
//   • Haupt-Render-Dispatcher      renderView()
//   • Projekt-Navigation           selectProj(), setUser(), onProjectChange()
//   • Modul-Leiste                 mfRefreshModuleBar(), mfSwitchAppModule()
//   • Benachrichtigungen           renderNotifs(), toggleNotif(), pushNotif()
//   • Login/Invite-Gates           renderMfLoginForm(), runMfInviteSetup()
//   • Upload-Workflow              uploadDatei(), onDateiAusgewaehlt()
//   • Neues Projekt Modal          openNewProjModal(), confirmNewProj()
//   • Viele Hilfsfunktionen        mfHtmlEscape(), downloadWandDatei(), ...
//
// TODO Cockpit-Umzug:
//   - messeflowNormalBoot() → Cockpit übernimmt Boot (kein Login-Gate)
//   - renderView() → in Cockpit-Routing einbinden (mfRenderModulePlaceholder)
//   - Topbar / Shell werden von CC Cockpit gerendert (body.ckp-module CSS)
//   - mfRefreshModuleBar() → Cockpit-Sidebar übernimmt Modul-Wechsel
// ═══════════════════════════════════════════════════════════════════════════════
...
/**
 * Zeilen für Druckrisiko-Hinweis / Confirm / druckFreigabe.gruende (Prüfserver fehler + hinweise + Kerndaten).
 * @param {{ fehler?: unknown, hinweise?: unknown, ergebnis?: Record<string, unknown>|null }} data
 * @returns {string[]}
 */
function mfBaueDruckRisikoListe(data) {
  const lines = [];
  const fe = Array.isArray(data.fehler) ? data.fehler : [];
  const hi = Array.isArray(data.hinweise) ? data.hinweise : [];
  fe.forEach((x) => {
  ...
  if (e && typeof e === 'object') {
    const dpi = /** @type {Record<string, unknown>} */ (e.dpi);
    if (dpi && typeof dpi.text === 'string' && dpi.text.trim()) {
      lines.push(`DPI: ${dpi.text.trim()}`);
    }
    const fb = /** @type {Record<string, unknown>} */ (e.farbraum);
    if (fb && typeof fb === 'object') {
      const mel = typeof fb.meldung === 'string' ? fb.meldung.trim() : '';
      const mod = typeof fb.modus === 'string' ? fb.modus.trim() : '';
      if (mel || mod) lines.push(`Farbraum: ${[mel, mod].filter(Boolean).join(' · ')}`);
    }
    const sc = /** @type {Record<string, unknown>} */ (e.schriften);
    if (sc && typeof sc === 'object' && typeof sc.hinweis === 'string' && sc.hinweis.trim()) {
      lines.push(`Schriften: ${sc.hinweis.trim()}`);
    }
  }
...
function refreshUserDropdown() {
  ...
  if (sel && sorted.includes(currentUserId)) sel.value = currentUserId;
}
...
/** Zuletzt im Upload-Modal gewähltes File (bleibt gültig, auch wenn <input type=file> den FileList-Eintrag verliert). */
let mfAktuelleUploadDatei = null;
let mfAktuelleUploadPidWid = '';
function mfClearAktuelleUploadDatei() {
  mfAktuelleUploadDatei = null;
  mfAktuelleUploadPidWid = '';
}
...
/** Antwort von `/pdf/pruefen` — vermeidet „Unexpected token '<'“ wenn die URL auf HTML (z. B. Cockpit-API) zeigt. */
async function mfParsePruefServerResponse(res) {
  const raw = await res.text();
  const t = raw.trimStart();
  if (!t) throw new Error('Leere Antwort vom Prüf-Server');
  if (t.startsWith('<')) {
    throw new Error(
      `HTML statt JSON (HTTP ${res.status}) — die Prüf-URL ist nicht der MesseFlow-PDF-Dienst. ` +
      'Im Cockpit: „Prüf-Server“ öffnen und z. B. `http://localhost:3030` eintragen (nicht die Cockpit-API).',
    );
  }
  let data;
  try {
    ...
    throw new Error('Ungültiges JSON — Prüf-Server-URL und Endpunkt /pdf/pruefen prüfen.');
  }
  if (!res.ok) {
    const msg = data && (data.fehler || data.message);
...
/**
 * Mappt Antworten von `pruefePdfDatei` (direkt :3030) auf das Legacy-Format `{ ok, ergebnis }`
 * für die bestehende Upload-Modal-Anzeige — keine erfundenen Messwerte.
 * @param {Record<string, unknown>} raw
 * @returns {{ ok: boolean, ergebnis: Record<string, unknown> }}
 */
function mfPruefAntwortAlsLegacyForm(raw) {
  if (!raw || typeof raw !== 'object') {
    throw new Error('Ungültige Antwort vom Prüf-Server.');
  }
  if (raw.ergebnis && typeof raw.ergebnis === 'object') {
    /** Prüfserver Phase 3: ok:false bei harten Druckfehlern, aber ergebnis liefern — nicht werfen. */
    const rawNorm = !raw.ok ? { ...raw, ok: true } : raw;
    const e = /** @type {Record<string, unknown>} */ (rawNorm.ergebnis);
    const am = e.abmessungen;
    if (am && typeof am === 'object') {
      const o = /** @type {Record<string, unknown>} */ (am);
      const bRaw = o.breiteMm != null ? o.breiteMm : o.breite_mm;
      const hRaw = o.hoeheMm != null ? o.hoeheMm : o.hoehe_mm;
      if (bRaw != null && hRaw != null) {
        const breiteMm = Number(bRaw);
        const hoeheMm = Number(hRaw);
        if (!Number.isNaN(breiteMm) && !Number.isNaN(hoeheMm)) {
          const masseStr =
            typeof e.masseStr === 'string' && e.masseStr.trim()
              ? e.masseStr
              : `${breiteMm} × ${hoeheMm} mm`;
          return {
            ...rawNorm,
  ...
  if (!raw.ok) {
    const fe = raw.fehler;
    const msg = Array.isArray(fe) ? fe.join(', ') : (typeof fe === 'string' ? fe : 'Prüfung fehlgeschlagen');
    throw new Error(msg || 'Prüfung fehlgeschlagen');
  }
  const hintRows = Array.isArray(raw.hinweise) ? raw.hinweise : [];
  const pruefungFromHints = hintRows.map((h, i) => {
    if (typeof h === 'string') {
      return { titel: `Hinweis ${i + 1}`, wert: h, detail: '', status: 'warnung' };
    }
    if (h && typeof h === 'object') {
      return {
        titel: h.titel || `Hinweis ${i + 1}`,
        wert: h.wert != null ? String(h.wert) : '—',
        detail: h.detail != null ? String(h.detail) : '',
        status: h.status === 'ok' || h.status === 'warnung' || h.status === 'fehler' ? h.status : 'warnung',
      };
    }
    return { titel: `Hinweis ${i + 1}`, wert: '—', detail: '', status: 'warnung' };
  });
  return {
    ok: true,
    ergebnis: {
      status: raw.status === 'geprueft' ? 'ok' : (typeof raw.status === 'string' ? raw.status : 'ok'),
      pruefung: pruefungFromHints,
      abmessungen: raw.abmessungen && typeof raw.abmessungen === 'object' ? raw.abmessungen : null,
      masseStr: typeof raw.masseStr === 'string' ? raw.masseStr : null,
      version: typeof raw.version === 'string' ? raw.version : '',
      alleEingebettet: raw.alleEingebettet !== false,
      nichtEingebettet: Array.isArray(raw.nichtEingebettet) ? raw.nichtEingebettet : [],
      meldung: raw.meldung != null ? String(raw.meldung) : null,
      istPdfX: !!raw.istPdfX,
      farbraum: raw.farbraum && typeof raw.farbraum === 'object' ? raw.farbraum : null,
      empfehlung: typeof raw.empfehlung === 'string' ? raw.empfehlung : null,
    },
  };
}

/**
 * PDF-Prüfung: bevorzugt `window.pruefePdfDatei` (direkt Prüfserver), sonst Legacy `mfPruefHttp` über CALDERA_SERVER.
 * @param {File|Blob} file
 * @returns {Promise<{ ok: boolean, ergebnis: Record<string, unknown> }>}
 */
async function mfPruefPdfDateiAufServer(file) {
  if (!file) {
    throw new Error('Keine Datei zum Prüfen.');
  }
  if (typeof pruefePdfDatei === 'function') {
    const raw = await pruefePdfDatei(file);
    return mfPruefAntwortAlsLegacyForm(raw);
  }
  const form = new FormData();
  const name = file instanceof File && file.name ? file.name : 'upload.pdf';
  form.append('datei', file, name);
  const res = await mfPruefHttp(CALDERA_SERVER + '/pdf/pruefen', {
    method: 'POST',
    body: form,
    signal: AbortSignal.timeout(15000),
  });
  return await mfParsePruefServerResponse(res);
}
...
async function onDateiAusgewaehlt(pid, wid, input){
  ...
  mfAktuelleUploadPidWid = `${pid}:${wid}`;
  mfAktuelleUploadDatei = raw;
  console.log('[MesseFlow Upload] file selected', raw && raw.name, raw && raw.size, raw instanceof File);
  ...
  if(btn){
    ...
    btn._pruefFehler        = [];
    btn._pruefHinweise      = [];
    btn._druckrisikoBestaetigt = false;
  ...
  // Wichtig: arrayBuffer() auf demselben File wie später für /pdf/pruefen kann den Blob-Stream leeren.
  // Deshalb nur auf einer Kopie (slice) lesen — Original `file` bleibt für Upload/Prüfung intakt.
  if(file.type === 'application/pdf' && btn){
    try {
      const pdfSource = typeof file.slice === 'function'
  ...
  const inputSnap = input.files && input.files[0];
  const fileToServer = (mfAktuelleUploadDatei && mfAktuelleUploadPidWid === `${pid}:${wid}`)
    ? mfAktuelleUploadDatei
    : inputSnap;
  if (!fileToServer || !(fileToServer instanceof Blob)) {
    if (btn) {
    ...
    if (el) {
      el.innerHTML = `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:9px 13px;font-size:12px;color:#991b1b;">
        Keine gültige Datei für die Prüfung — bitte erneut auswählen.</div>`;
    }
    toast('Hinweis', 'Keine Datei zum Prüfen (bitte erneut auswählen).', 'ty');
    return;
  }
  console.log('[MesseFlow Prüfung] file to server', fileToServer && fileToServer.name, fileToServer && fileToServer.size, fileToServer instanceof File, fileToServer instanceof Blob);

  try {
    const data = await mfPruefPdfDateiAufServer(fileToServer);
    if (!data.ok && !(data.ergebnis && typeof data.ergebnis === 'object')) {
      const fe = data.fehler;
      throw new Error(Array.isArray(fe) ? fe.join(', ') : (fe || 'Prüfung fehlgeschlagen'));
    }
    const e = data.ergebnis;
    const serverFehler = Array.isArray(data.fehler) ? data.fehler : [];
    const serverHinweise = Array.isArray(data.hinweise) ? data.hinweise : [];
    const hatDruckRisiko = serverFehler.length > 0 || serverHinweise.length > 0;
    const risikoListeKopf = mfBaueDruckRisikoListe({
      fehler: serverFehler,
      hinweise: serverHinweise,
      ergebnis: e && typeof e === 'object' ? e : null,
    });
    ...
    if(btn){
      ...
      btn._dateiMasse         = e.abmessungen
        ...
        : null;
      btn._pruefFehler        = serverFehler;
      btn._pruefHinweise      = serverHinweise;
      btn.dataset.analysiert  = 'ja';
      btn.dataset.serverOffline = 'nein';
    }

    // ── Maß-Vergleich ZUERST berechnen (brauchen wir für Farbe) ──────────────
    const p0 = getP(pid), w0 = getW(p0, wid);
    const fmasseStr = e.abmessungen ? `${e.abmessungen.breiteMm} × ${e.abmessungen.hoeheMm} mm` : '';
    const vglModal = (w0.bestellmass && fmasseStr) ? vergleicheMasse(w0.bestellmass, fmasseStr) : null;

    // Farbe der Maß-Anzeige: grün wenn OK, gelb bei Warnung, rot bei Abweichung
    const massVglStufe = vglModal?.stufe || 'ok';
    const massFarbe = massVglStufe === 'ok' ? 'var(--green)'
                    : massVglStufe === 'warnung' ? '#92400e'
                    : 'var(--red)';
          ...
          <div style="background:${massBg};border:1px solid ${massBd};border-radius:7px;
            ...
            <div>
              <div style="font-weight:700;font-size:14px;color:${massFarbe};">
                ${massIcon}Dateimaß erkannt: ${a.breiteMm} × ${a.hoeheMm} mm
              </div>
    ...
    // Client-Status: Server-Ergebnis mit unseren Overrides neu berechnen
    // (DPI immer 'ok' da wir Vektor/Raster nicht zuverlässig trennen können;
    //  Maße überschreiben mit eigenem Vergleich)
    const clientPruefung = (e.pruefung||[]).map(z => {
      const t = (z.titel||'').toLowerCase();
      // Maße: eigener Client-Vergleich überschreibt Server
      if ((t.includes('maß') || t.includes('mass') || t.includes('abmessung')) && vglModal) {
        const s = vglModal.stufe === 'ok' ? 'ok' : vglModal.stufe === 'warnung' ? 'warnung' : 'fehler';
        return { ...z, status: s };
      }
      // DPI: Server-Status direkt übernehmen (nicht überschreiben)
      return z;
    });
    const clientStatus = clientPruefung.reduce((worst, z) => {
      if (z.status === 'fehler') return 'fehler';
      if (z.status === 'warnung' && worst !== 'fehler') return 'warnung';
      return worst;
    }, 'ok');

    const massFehler = !!(vglModal && vglModal.stufe === 'abweichung');
    let slotModal = clientStatus;
    if (hatDruckRisiko && !massFehler && clientStatus === 'fehler') {
      slotModal = 'warnung';
    }

    let gesBg = slotModal === 'ok' ? '#f0fdf4' : slotModal === 'warnung' ? '#fffbeb' : slotModal === 'fehler' ? '#fef2f2' : '#f9fafb';
    let gesBd = slotModal === 'ok' ? '#86efac' : slotModal === 'warnung' ? '#fde68a' : slotModal === 'fehler' ? '#fecaca' : 'var(--line)';
    let gesCol = slotModal === 'ok' ? 'var(--green)' : slotModal === 'warnung' ? '#92400e' : slotModal === 'fehler' ? 'var(--red)' : 'var(--muted)';
    let gesIcon = slotModal === 'ok' ? '✓' : slotModal === 'warnung' ? '⚡' : slotModal === 'fehler' ? '✖' : 'ℹ';
    let gesText =
      !hatDruckRisiko && slotModal === 'ok'
        ? '✅ Datei OK – hochladen möglich'
        : hatDruckRisiko && massFehler
          ? '✖ Kritische Maßabweichung und Druckrisiken – prüfen; Upload nur mit Bestätigung.'
        : hatDruckRisiko
          ? '⚠ Datei hat Druckrisiken. Upload mit Bestätigung – nur auf eigene Verantwortung.'
          : slotModal === 'ok'
            ? 'Datei OK – bereit für Upload'
            : slotModal === 'warnung'
              ? 'Warnung – bitte prüfen'
              : slotModal === 'fehler'
                ? 'Datei nicht OK – bitte korrigieren'
                : 'Nicht geprüft';

    if (hatDruckRisiko && !massFehler) {
      gesBg = '#fffbeb';
      gesBd = '#fde68a';
      gesCol = '#92400e';
      gesIcon = '⚠';
    ...
    // Pixel-Felder entfernt → DPI-Prüfung immer deaktiviert
    const hatManuellePixel = false;

    // Warnung-Typen sammeln (für Risiko-Button-Entscheidung)
    let hatEchteWarnung = false;

    const zeilen = (e.pruefung||[]).map(z=>{
      let { status, titel, wert, detail } = z;

      // ── Maße-Zeile: Client-Vergleich hat Vorrang vor Server-Ergebnis ────────
      // Der Server kennt das Bestellmaß nicht → Vergleich nur client-seitig möglich.
      ...
      if (isMassZeile && vglModal) {
        status = vglModal.stufe === 'ok' ? 'ok'
               : vglModal.stufe === 'warnung' ? 'warnung'
               : 'fehler';
        wert   = fmasseStr || wert;
      ...
      // ── DPI-Zeile: Server-Ergebnis direkt anzeigen ──────────────────────────
      ...
      if (isDpiZeile) {
        if (status === 'ok') {
          return `<tr style="background:#f0fdf4;">
            ...
            <td style="padding:5px 10px;font-size:13px;font-weight:700;color:var(--green);">✓ ${wert}</td>
            <td style="padding:5px 10px;font-size:11px;color:#64748b;">${detail||''}</td>
          </tr>`;
        }
        // warnung oder fehler → anzeigen + Risiko-Button aktivieren
        hatEchteWarnung = true;
        const dpiIc  = status === 'warnung' ? '⚡' : '✖';
        const dpiCl  = status === 'warnung' ? '#92400e'  : 'var(--red)';
        const dpiBg  = status === 'warnung' ? 'background:#fffbeb;' : 'background:#fef2f2;';
        return `<tr style="${dpiBg}">
          <td style="padding:5px 10px;font-size:12px;color:var(--muted);white-space:nowrap;width:90px;">${titel}</td>
          <td style="padding:5px 10px;font-size:13px;font-weight:700;color:${dpiCl};">${dpiIc} ${wert}</td>
          <td style="padding:5px 10px;font-size:11px;color:#64748b;">${detail||'Auflösung zu niedrig für Großformatdruck'}</td>
        </tr>`;
      }
      // ────────────────────────────────────────────────────────────────────────
    ...
    if(btn){ btn.disabled=false; btn.textContent='📤 Hochladen & prüfen'; }

    // ── Risiko-Button: zeigen wenn echte Warnungen oder Prüfserver-Druckrisiko ─
    const risikoBtn = document.getElementById(`risiko-btn-${wid}`);
    if (risikoBtn) {
      if (hatEchteWarnung || hatDruckRisiko) {
        risikoBtn.style.display = '';
        const warnListe = (e.pruefung||[])
          .filter(z => z.status === 'warnung' || z.status === 'fehler')
          .map(z => `${z.titel}: ${z.wert}`).join(' · ');
        const serverTeil = risikoListeKopf.join(' · ');
        const mix = [serverTeil, warnListe].filter(Boolean).join(' · ');
        risikoBtn.textContent = `⚠ Trotzdem hochladen – auf eigenes Risiko (${mix || 'Hinweise vorhanden'})`;
        risikoBtn.title = 'Kunde bestätigt: Datei wird trotz Druckrisiken hochgeladen';
      } else {
        risikoBtn.style.display = 'none';
      }
    }
    // ────────────────────────────────────────────────────────────────────────
...
function confirmUpload(pid, wid, isRisiko){
  ...
  const fe = Array.isArray(btn._pruefFehler) ? btn._pruefFehler : [];
  const hi = Array.isArray(btn._pruefHinweise) ? btn._pruefHinweise : [];
  const hatDruckRisiko = fe.length > 0 || hi.length > 0;
  let risikoListe = mfBaueDruckRisikoListe({ fehler: fe, hinweise: hi, ergebnis: fontE });
  const prWarnRows = (fontE?.pruefung || []).filter((z) => z && (z.status === 'warnung' || z.status === 'fehler'));
  if (!risikoListe.length && (isRisiko || prWarnRows.length)) {
    risikoListe = prWarnRows.map((z) => {
      const t = String(z.titel || 'Check');
      const v = String(z.wert || '');
      const d = z.detail ? ` — ${String(z.detail)}` : '';
      return `${t}: ${v}${d}`.trim();
    });
  }
  const needsDruckBestaetigung = hatDruckRisiko || isRisiko;
  btn._druckrisikoBestaetigt = false;
  if (needsDruckBestaetigung) {
    const body = risikoListe.length ? risikoListe.join('\n') : '(Druckrisiken aus Prüfung)';
    const ok = confirm(
      'Diese Datei hat Druckrisiken:\n\n' +
      body +
      '\n\nDer Druck kann fehlerhaft sein.\n' +
      'Trotzdem auf eigene Verantwortung freigeben?'
    );
    if (!ok) return;
    btn._druckrisikoBestaetigt = true;
  }
  ...
  // Nur kleine Metadaten fürs Workspace — keine Base64-Vorschau (sonst PUT „entity too large“).
  w.dateiMeta = {
    dateiname: file.name,
    dateigroesse: file.size,
    mimeType: file.type || 'application/pdf',
    pruefstatus: fontE ? fontE.status : null,
    hinweise: Array.isArray(fontE?.pruefung)
      ? fontE.pruefung.filter(z => z && z.status === 'warnung').slice(0, 20).map(z => ({ titel: z.titel, wert: z.wert, detail: z.detail || '' }))
      : [],
    fehler: Array.isArray(fontE?.pruefung)
      ? fontE.pruefung.filter(z => z && z.status === 'fehler').slice(0, 20).map(z => ({ titel: z.titel, wert: z.wert, detail: z.detail || '' }))
      : [],
    serverFehler: fe.slice(0, 40),
    serverHinweise: hi.slice(0, 40),
    abmessungen: dateiMasse ? { breiteMm: dateiMasse.breite, hoeheMm: dateiMasse.hoehe } : null,
  };

  w.druckFreigabe = btn._druckrisikoBestaetigt
    ? {
        risiko: !!(hatDruckRisiko || isRisiko),
        freigegebenAufEigeneVerantwortung: true,
        zeit: new Date().toISOString(),
        gruende: risikoListe.length ? risikoListe.slice(0, 60) : ['(Prüfung mit Bestätigung)'],
        kommentar: 'Kunde hat Druckfreigabe trotz Prüfhinweisen bestätigt.',
      }
    : {
        risiko: false,
        zeit: new Date().toISOString(),
        kommentar: 'Datei ohne relevante Druckrisiken geprüft.',
      };

  // Vorschau nur im RAM (SVG bis Reload); nicht in Workspace serialisieren.
  ...
  addFileToWall(pid, wid, {
    ...
    // Risiko-Upload: Nutzer hat Druckrisiko-Bestätigung (Confirm) durchlaufen
    risikoUpload:   btn?._druckrisikoBestaetigt === true,
    risikoBestaetigt: btn?._druckrisikoBestaetigt === true
...
// ═══════════════════════════════════════════════════════
// PROJEKT-TEAM VERWALTUNG (Phase 1C: sichtbare Benutzer-/Rollen-Zuweisungs-UI entfernt)
// ═══════════════════════════════════════════════════════
...
async function dateiNochmalPruefen(pid, wid){
  ...
  try {
    const data = await mfPruefPdfDateiAufServer(gespeichert.blob);
    if (!data.ok) {
      const fe = data.fehler;
      throw new Error(Array.isArray(fe) ? fe.join(', ') : (fe || 'Prüfung fehlgeschlagen'));
    }
...
function openNewProjModal(){
  if (typeof window.mfMayCreateMesseflowProject === 'function' && !window.mfMayCreateMesseflowProject()) {
    const inCockpit = typeof window !== 'undefined' && window.__MF_COCKPIT_EMBED;
    toast(
      'Keine Berechtigung',
      inCockpit
        ? 'Erforderlich: ccintern.messeflow.erstellen (Cockpit-Benutzerrechte, GET /auth/my-rights).'
        : 'Nur Administrator, CC Intern oder Zwischenhändler können ein neues Projekt anlegen.',
      'ty',
    );
    return;
  }
  ...
  const agenturFirmen = FIRMS.filter(f => String(f?.typ || '').toLowerCase() === 'agentur');
  const firmaCandidates = agenturFirmen.length > 0 ? agenturFirmen : FIRMS;
  const agenturOpts = firmaCandidates
  ...
  const html =
    ...
    row2(fg('Bestelldatum', inp('np-bestell','', 'date')),
...
function confirmNewProj(){
  ...
  const newP = {
    ...
    waende,
    auftragsInfo: {
      bestelldatum:    document.getElementById('np-bestell')?.value || '',
      liefertermin:    dl,
      kunde,
      projektname:     name,
...
// INIT
// ═══════════════════════════════════════════════════════
/** Cockpit-Embed: Sidebar-Buttons nach GET /auth/my-rights (`ccintern.messeflow.*`). */
function mfApplyCockpitMesseflowUiGates() {
  if (typeof window === 'undefined' || !window.__MF_COCKPIT_EMBED) return;
  const mayCreate = typeof window.mfMayCreateMesseflowProject === 'function' && window.mfMayCreateMesseflowProject();
  const mayImport = typeof window.mfMayImportMesseflowExcel === 'function' && window.mfMayImportMesseflowExcel();
  const add = document.getElementById('add-proj');
  if (add) add.style.display = mayCreate ? '' : 'none';
  const xl = document.getElementById('mf-sidebar-excel-btn');
  if (xl) xl.style.display = mayImport ? '' : 'none';
}

/** Keys (lowercase) und Namen, die nie im Workspace-PUT landen dürfen. */
const MF_WS_STRIP_KEYS = new Set([
  'dateipreview', 'previewurl', 'objecturl', 'pdfpreview', 'thumbnail', 'base64',
  'datenurl', 'dataurl', 'blob', '_previewurl', '_datei', 'pdfdata', 'filecontent',
  'imagepreview', 'previewdata', 'rawpdf', 'buffer',
]);

function mfWorkspaceKeyStripped(key) {
  if (key == null || key === '') return false;
  const lo = String(key).toLowerCase();
  if (MF_WS_STRIP_KEYS.has(lo)) return true;
  if (lo.includes('thumbnail')) return true;
  if ((lo.includes('preview') || lo.includes('thumb')) && (lo.includes('url') || lo.includes('data') || lo.includes('base64'))) return true;
  return false;
}

function mfWorkspaceStringLikelyBase64(s) {
  if (typeof s !== 'string' || s.length < 4096) return false;
  const t = s.replace(/\s+/g, '');
  if (t.length < 4096) return false;
  return /^[A-Za-z0-9+/]+=*$/.test(t.slice(0, 200)) && /^[A-Za-z0-9+/]+=*$/.test(t.slice(-200));
}

/**
 * Tiefe Bereinigung für Workspace-JSON (keine File/Blob/AB, keine data:-URLs, keine Riesen-Base64-Strings).
 * @param {unknown} value
 * @returns {unknown}
 */
function mfSanitizeWorkspaceDeep(value) {
  ...
  if (typeof value === 'string') {
    ...
    if (mfWorkspaceStringLikelyBase64(value)) return undefined;
    return value;
  }
  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      const nv = mfSanitizeWorkspaceDeep(value[i]);
  ...
  for (const k of Object.keys(value)) {
    if (mfWorkspaceKeyStripped(k)) continue;
    const nv = mfSanitizeWorkspaceDeep(/** @type {Record<string, unknown>} */ (value)[k]);
...
/**
 * Gesamten MesseFlow-Workspace-Body vor PUT bereinigen (projects + notifs + auditLog).
 * @param {unknown} payload
 * @returns {{ projects: unknown[]; notifs: unknown[]; auditLog: unknown[] }}
 */
function mfSanitizeMesseflowWorkspacePayload(payload) {
  if (!payload || typeof payload !== 'object') {
    return { projects: [], notifs: [], auditLog: [] };
  }
  ...
  try {
    const cleaned = mfSanitizeWorkspaceDeep(raw);
    if (!cleaned || typeof cleaned !== 'object') {
      return { projects: [], notifs: [], auditLog: [] };
    }
    const c = /** @type {Record<string, unknown>} */ (cleaned);
    return {
      projects: Array.isArray(c.projects) ? c.projects : [],
      notifs: Array.isArray(c.notifs) ? c.notifs : [],
      auditLog: Array.isArray(c.auditLog) ? c.auditLog : [],
    };
  } catch (e) {
    console.warn('[MF] mfSanitizeMesseflowWorkspacePayload:', e);
    return { projects: [], notifs: [], auditLog: [] };
  }
}
 ...
 */
function mfSanitizeMesseflowWorkspaceProjects(projects) {
  return mfSanitizeMesseflowWorkspacePayload({
...
function mfBuildWorkspacePayload() {
  const projectsRaw = Array.isArray(MesseFlowState?.projects) ? MesseFlowState.projects : [];
  const notifs = Array.isArray(MesseFlowState?.notifs) ? MesseFlowState.notifs : [];
  const auditLog = Array.isArray(MesseFlowState?.auditLog) ? MesseFlowState.auditLog : [];
  return mfSanitizeMesseflowWorkspacePayload({ projects: projectsRaw, notifs, auditLog });
}

let mfSaveInFlight = false;
let mfSaveQueued = false;
let mfAutosaveTimer = null;
let mfLastSavedSignature = '';
let mfLastAutosaveSignature = '';

async function mfSaveState() {
  const inCockpit = typeof window !== 'undefined' && window.__MF_COCKPIT_EMBED;
  if (!inCockpit) return;
  if (typeof window.__mfSaveMesseflowWorkspace !== 'function') return;
  ...
  mfSaveInFlight = true;
  try {
    await window.__mfSaveMesseflowWorkspace();
    try {
      mfLastSavedSignature = JSON.stringify(mfBuildWorkspacePayload());
    } catch {
      mfLastSavedSignature = '';
    }
  } catch (err) {
    console.error('[MF] save workspace failed:', err);
  } finally {
    mfSaveInFlight = false;
    if (mfSaveQueued) {
      mfSaveQueued = false;
      void mfSaveState();
    }
...
function mfStartAutosaveWatcher() {
  if (mfAutosaveTimer != null) return;
  mfAutosaveTimer = window.setInterval(() => {
    try {
      const sig = JSON.stringify(mfBuildWorkspacePayload());
      if (sig === mfLastAutosaveSignature) return;
      mfLastAutosaveSignature = sig;
      if (sig !== mfLastSavedSignature) void mfSaveState();
    } catch {
...
async function mfLoadWorkspaceIntoState() {
  const inCockpit = typeof window !== 'undefined' && window.__MF_COCKPIT_EMBED;
  if (!inCockpit) return;
  if (typeof window.__mfLoadMesseflowWorkspace !== 'function') return;
  try {
    const ws = await window.__mfLoadMesseflowWorkspace();
    if (!ws || typeof ws !== 'object') return;
    MesseFlowState.projects = Array.isArray(ws.projects) ? ws.projects : [];
    MesseFlowState.notifs = Array.isArray(ws.notifs) ? ws.notifs : [];
    MesseFlowState.auditLog = Array.isArray(ws.auditLog) ? ws.auditLog : [];
    mfLastSavedSignature = JSON.stringify(mfBuildWorkspacePayload());
    mfLastAutosaveSignature = mfLastSavedSignature;
  } catch (err) {
    console.error('[MF] load workspace failed:', err);
  }
}

if (typeof window !== 'undefined') {
  window.mfSaveState = mfSaveState;
  window.mfSanitizeMesseflowWorkspaceProjects = mfSanitizeMesseflowWorkspaceProjects;
  window.mfSanitizeMesseflowWorkspacePayload = mfSanitizeMesseflowWorkspacePayload;
}
...
function messeflowNormalBoot(preferredUserId) {
  ...
  // Cockpit-Kontext: kein Demo-User 'u_celal' mehr – auf ersten verfügbaren User zurückfallen
  const bootUser = preferredUserId || (USERS[0] ? USERS[0].id : '');
  setUser(bootUser);
  renderSidebar();
  updateBadge();
  const u = getCurrentUser();
  ...
  if (first) selectProj(first.id);
    ...
    if (view) {
      const inCockpit = typeof window !== 'undefined' && window.__MF_COCKPIT_EMBED;
      const mayCreate =
        typeof window.mfMayCreateMesseflowProject === 'function' && window.mfMayCreateMesseflowProject();
      const mayImport =
        typeof window.mfMayImportMesseflowExcel === 'function' && window.mfMayImportMesseflowExcel();
      const isInternalRole = (role === 'admin' || role === 'cc_intern');
      let emptyHint = '';
      if (inCockpit) {
        if (mayCreate && mayImport) {
          emptyHint = '← Kein Projekt vorhanden – legen Sie links ein neues Projekt an oder importieren Sie Excel.';
        } else if (mayCreate) {
          emptyHint = '← Kein Projekt vorhanden – legen Sie links ein neues Projekt an.';
        } else if (mayImport) {
          emptyHint = '← Kein Projekt vorhanden – importieren Sie links eine Excel-Datei.';
        }
      } else if (isInternalRole) {
        emptyHint = '← Kein Projekt vorhanden – legen Sie links ein neues Projekt an oder importieren Sie Excel.';
      }
      view.innerHTML = emptyHint
        ? `<div style="color:var(--muted);text-align:center;padding:60px 0;font-size:15px;">${emptyHint}</div>`
  ...
  if (typeof window !== 'undefined' && window.__MF_COCKPIT_EMBED) {
    mfStartAutosaveWatcher();
  }
  if (typeof refreshRoleSelVisibility === 'function') refreshRoleSelVisibility();
  mfApplyCockpitMesseflowUiGates();
}

// ── Test-Modus: Boot ohne Login bleibt in messeflowEntry; keine sichtbare Test-/Rollen-Chrome (Phase 1A). ──
...
// ════════════════════════════════════════════════════════════════════════════
// COCKPIT-INTEGRATION: messeflowCockpitBoot(userId, container)
// ════════════════════════════════════════════════════════════════════════════
// Wird von Cockpit aufgerufen statt messeflowNormalBoot().
// Cockpit übernimmt Auth → kein Login-Gate, kein mf_session_v1 nötig.
//
// Aufruf aus Cockpit:
//   await mfLoadCockpitData(usersResp, firmenResp);  // Daten laden
//   messeflowCockpitBoot(cockpitUserId, document.getElementById('mf-mount'));

/**
 * Startet MesseFlow im Cockpit-Kontext.
 * @param {string} cockpitUserId  — UUID des eingeloggten Cockpit-Benutzers
 * @param {HTMLElement} [container] — DOM-Element in das MesseFlow gerendert wird
 *                                    (Fallback: document.getElementById('view'))
 */
async function messeflowCockpitBoot(cockpitUserId, container) {
  const gate = document.getElementById('mf-login-gate');
  if (gate) gate.style.display = 'none';
  const inv = document.getElementById('mf-invite-gate');
  if (inv) inv.style.display = 'none';

  if (typeof mfSetMainChromeVisible === 'function') {
    mfSetMainChromeVisible(true);
  }

  let resolved = String(cockpitUserId || '').trim();
  if (resolved && typeof USERS !== 'undefined' && !USERS.some(u => u.id === resolved)) {
    const admin = USERS.find(u => u.rolle === 'admin');
    resolved = admin ? admin.id : (USERS[0] ? USERS[0].id : '');
  } else if (!resolved && typeof USERS !== 'undefined') {
    const admin = USERS.find(u => u.rolle === 'admin');
    resolved = admin ? admin.id : (USERS[0] ? USERS[0].id : '');
  }

  if (!resolved) {
    console.error('[MF] Cockpit-Boot: kein gültiger Benutzer (USERS leer oder JWT sub unbekannt).');
    const view = document.getElementById('view');
    if (view) {
      view.innerHTML = '<p class="ckp-api-error" role="alert">MesseFlow: Keine Benutzerdaten geladen. Bitte Seite neu laden oder Administrator informieren.</p>';
    }
    return;
  }

  await mfLoadWorkspaceIntoState();

  if (typeof messeflowNormalBoot === 'function') {
    messeflowNormalBoot(resolved);
  } else {
    console.error('[MF] messeflowNormalBoot nicht verfügbar');
  }

  console.log('[MF] Cockpit-Boot abgeschlossen — User:', resolved);
}

if (typeof window !== 'undefined') {
  window.messeflowCockpitBoot = messeflowCockpitBoot;
}
// ════════════════════════════════════════════════════════════════════════════

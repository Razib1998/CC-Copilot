...
(function() {
  ...
  function load(callback) {
    ...
    ds.loadAsync(KEY, null, function(err, data) {
      if (!err && Array.isArray(data) && data.length > 0) {
        ...
        if (g && Array.isArray(g.angebote) && g.angebote.length) {
          AG_DATEN.length = 0;
          g.angebote.forEach(function (item) { AG_DATEN.push(JSON.parse(JSON.stringify(item))); });
          console.info('[Angebote] ' + g.angebote.length + ' Angebot(e) aus Seed (CCinter_COCKPIT_UMZUG/daten)');
        } else {
  ...
  function init() {
    ...
    load(function(/* loaded */) {
      // Immer nach load (auch Seed-Pfad) — sonst bleibt die Angebots-Liste leer
      if (typeof renderAngebote === 'function') renderAngebote();
    });
  }

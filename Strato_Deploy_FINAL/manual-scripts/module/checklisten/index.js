...
// ─────────────────────────────────────────────────────────────────────
// Cockpit: Persistenz ausschließlich über cockpitApi.saveChecklistenVorlagenToApi
//   → /api/v1/checklisten (kein paralleler DataService-Cache für Vorlagen).
// Legacy (ohne Cockpit-Mount): DataService-Key 'cc_intern_cl_vorlagen_v1'.
//
// CL_VORLAGEN = Laufzeit-Cache derselben API-Daten (siehe reloadChecklistenVorlagenFromApi).
// ══════════════════════════════════════════════════════════════════════
...
(function() {
  ...
  function _installWraps() {
    ...
    window.clSavePunkt = function() {
      ...
      if (typeof window !== 'undefined' && window.__CL_SKIP_VORLAGEN_SAVE_ONCE) {
        window.__CL_SKIP_VORLAGEN_SAVE_ONCE = false;
        return r;
      }
      save(); return r;
    };
    ...
    window.clDeletePunkt = function(vorlageId, idx) {
      // ccInternConfirm ist asynchron — kein save() hier (Persistenz im onYes-Callback der View).
      return typeof _origClDeletePunkt === 'function' ? _origClDeletePunkt(vorlageId, idx) : undefined;
    };

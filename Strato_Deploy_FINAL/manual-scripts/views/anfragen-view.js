...
//
// Backend: GET|POST|PUT|DELETE `/api/v1/ccintern/anfragen` via `apiFetch` (+ `x-project-id`).
// ════════════════════════════════════════════════════════════════════
...
window.anfSpeichern = function () {
  ...
  if (!kunde) {
    showToast('⚠ Bitte Kundenname eingeben');
    return;
  }

  const r = berechneAngebot(anfParams);
  const leistungLabels = {
    fahrzeug: '🚗 PKW / Fahrzeug',
    fenster: '🪟 Fenster',
    schild: '📋 Schild',
    druck: '🖨️ Druck',
    aufkleber: '🏷 Aufkleber',
  ...
  const ll = leistungLabels[anfParams.leistung] || anfParams.leistung;

  function runCrmHook(idLabel) {
    var crmNeuAngelegt = false;
    var crmKey = kunde.split(' ')[0];
    ...
    if (!existiert) {
      ...
      CRM_KUNDEN[crmKey] = {
        ...
        naechsteAktion: 'Angebot ' + idLabel + ' nachfassen',
        notiz: 'Über Schnell-Angebot ' + idLabel + ' angelegt. Leistung: ' + ll,
      };
      crmNeuAngelegt = true;
    }
    return crmNeuAngelegt;
  }

  if (window.__CCINTERN_COCKPIT_MOUNT__ && typeof anfPostAnfrage === 'function') {
    var draft = {
      kunde: kunde,
      kontakt: kontakt,
    ...
    var body = {
      betreff: 'Schnellanfrage · ' + kunde,
      beschreibung: anfBuildPayloadDescription(draft),
      status: 'offen',
      kunde_id: null,
    };
    anfPostAnfrage(body)
      .then(function (row) {
        var ui = anfApiRowToUi(row);
        if (!ui || !ui.id) throw new Error('Ungültige API-Antwort');
        ANF_DATEN.unshift(ui);
        var idLabel = ui.anfragen_nr && String(ui.anfragen_nr).trim() !== '' ? ui.anfragen_nr : ui.id;
        var crmNeu = runCrmHook(idLabel);
        anfModalClose();
        window.renderAnfragen();
        anfOpenDetail(ui.id);
        var msg = '✓ ' + idLabel + ' · ' + kunde + ' · € ' + r.summeNetto.toFixed(0);
        if (crmNeu) msg += ' · Kunde im CRM angelegt';
        showToast(msg);
      })
      .catch(function (err) {
  ...
  var crmNeuAngelegt = runCrmHook(id);
...
window.renderAnfragen = function () {
  console.log('[anfragen-view] renderAnfragen called', window.__CCINTERN_COCKPIT_MOUNT__);
  /** Mobile Shell: kein Schnell-Anfragen-Listen-GET beim Start (Desktop-Modul); keine Sonder-API, nur Wegfall des unnötigen Calls. */
  if (window.__CCINTERN_MITARBEITER_APP_BOOT__) {
    renderAnfragenDom();
    return;
  }
  if (window.__CCINTERN_COCKPIT_MOUNT__) {
    if (!__anfListInflight) {
      __anfListInflight = anfReloadListeFromApi()
        .then(function () {
          __anfHydratedFromApi = true;
        })
        .finally(function () {
          __anfListInflight = null;
        });
    }
    __anfListInflight
      .then(function () {
      ...
      .catch(function (err) {
        if (typeof showToast === 'function') {
          showToast('⚠ Anfragen laden: ' + (err && err.message ? err.message : String(err)));
        }
        if (typeof window.renderAnfragen === 'function') window.renderAnfragen();
      });
    return;
  }
  renderAnfragenDom();
...
var __anfHydratedFromApi = false;
var __anfListInflight = null;

// Auth-Modul wird vom Cockpit-Bridge VOR dem Laden klassischer Skripte unter
// `window.CCIntern.auth` bereitgestellt. KEIN dynamisches `import(...)` mehr —
// sonst transformiert Vite diese Datei zu einem ES-Modul und das classic-<script>
// scheitert mit "Cannot use import statement outside a module".
function anfGetAuthModule() {
  var auth =
    typeof window !== 'undefined' && window.CCIntern && window.CCIntern.auth
  ...
  return Promise.reject(
    new Error('apiFetch-Modul nicht gefunden (window.CCIntern.auth fehlt).'),
  );
}
function anfApiFetch(path, options) {
  console.log('[anfragen-view] anfApiFetch start', path, options && options.method);
  options = options || {};
  return anfGetAuthModule().then(function (mod) {
    console.log('[anfragen-view] anfApiFetch after anfGetAuthModule', Boolean(mod), mod && typeof mod.apiFetch);
    function runFetch() {
      var headers = Object.assign({}, options.headers || {});
      var pid = typeof mod.getCurrentProjectId === 'function' ? mod.getCurrentProjectId() : '';
      if (pid && String(pid).trim() !== '') {
        headers['x-project-id'] = String(pid).trim();
      }
      console.log('[anfragen-view] anfApiFetch before mod.apiFetch', path, Object.keys(headers));
      return mod.apiFetch(path, Object.assign({}, options, { headers: headers }));
    }
    var pid0 = typeof mod.getCurrentProjectId === 'function' ? mod.getCurrentProjectId() : '';
    if (pid0 && String(pid0).trim() !== '') {
      return runFetch();
    }
    var hydrate = mod.hydrateCockpitAccessibleProjectsAndEnsureContext;
    if (typeof hydrate === 'function') {
      return hydrate().then(runFetch);
    }
    return runFetch();
  });
}
function anfPickPayload(data) {
  if (!data || typeof data !== 'object') return data;
  if (data.data !== undefined && data.success === true) return data.data;
  return data;
}
function anfPickAnfragenList(data) {
  var d = anfPickPayload(data);
  if (!d) return [];
  if (Array.isArray(d.anfragen)) return d.anfragen;
  if (d.data && Array.isArray(d.data.anfragen)) return d.data.anfragen;
  return [];
}
function anfPickAnfrage(data) {
  var d = anfPickPayload(data);
  if (!d || typeof d !== 'object') return null;
  if (d.anfrage && typeof d.anfrage === 'object') return d.anfrage;
  if (d.data && d.data.anfrage && typeof d.data.anfrage === 'object') return d.data.anfrage;
  return null;
}
function anfIsApiUuid(id) {
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(id || ''));
}
function anfApiStatusFromUi(st) {
  if (st === 'gewonnen') return 'erledigt';
  if (st === 'angebot') return 'in_bearbeitung';
  if (st === 'abgelehnt') return 'abgelehnt';
  return 'offen';
}
function anfUiStatusFromApi(st) {
  if (st === 'erledigt') return 'gewonnen';
  if (st === 'in_bearbeitung') return 'angebot';
  if (st === 'abgelehnt') return 'abgelehnt';
  if (st === 'offen') return 'offen';
  return 'offen';
}
function anfFormatDe(iso) {
  if (!iso) return '';
  try {
    var d = new Date(iso);
...
function anfBuildPayloadDescription(a) {
  return JSON.stringify({
    v: 1,
    kontakt: a.kontakt || '',
    kanal: a.kanal || 'Telefon',
    leistung: a.leistung || 'fahrzeug',
    leistungLabel: a.leistungLabel || '',
    beschr: a.beschr || '',
    params: a.params || null,
    notiz: a.notiz || '',
  });
}
function anfApiRowToUi(row) {
  ...
  var betreff = row.betreff != null ? String(row.betreff) : '';
  var kundeFromBetreff = betreff.replace(/^Schnellanfrage\s*[·•]\s*/i, '').trim();
  var ui = {
    id: String(row.id || ''),
    anfragen_nr: row.anfragen_nr != null ? String(row.anfragen_nr) : '',
    kunde: row.kunde_name ? String(row.kunde_name) : kundeFromBetreff || '—',
    kontakt: extra.kontakt || '',
    kanal: extra.kanal || 'Telefon',
    leistung: extra.leistung || 'fahrzeug',
    leistungLabel: extra.leistungLabel || '—',
    beschr: extra.beschr || '',
    params: extra.params || null,
    notiz: extra.notiz || '',
    status: anfUiStatusFromApi(row.status),
    erstellt: anfFormatDe(row.erstellt_am),
  };
  return ui;
}
function anfReloadListeFromApi() {
  console.log('[anfragen-view] anfReloadListeFromApi start');
  return anfApiFetch('/api/v1/ccintern/anfragen?limit=200&page=1', { method: 'GET' }).then(function (raw) {
    var rows = anfPickAnfragenList(raw);
    if (typeof ANF_DATEN === 'undefined') return;
    ANF_DATEN.length = 0;
    rows.forEach(function (row) {
      var ui = anfApiRowToUi(row);
      if (ui && ui.id) ANF_DATEN.push(ui);
    });
  });
}
function anfPostAnfrage(body) {
  return anfApiFetch('/api/v1/ccintern/anfragen', { method: 'POST', body: body }).then(function (raw) {
    return anfPickAnfrage(raw);
  });
}
function anfPutUiRowApi(a, apiStatus) {
  var body = {
    betreff: 'Schnellanfrage · ' + String(a.kunde || '—').trim(),
    beschreibung: anfBuildPayloadDescription(a),
    status: apiStatus,
    kunde_id: null,
  };
  return anfApiFetch('/api/v1/ccintern/anfragen/' + encodeURIComponent(String(a.id)), {
    method: 'PUT',
    body: body,
  }).then(function (raw) {
    var row = anfPickAnfrage(raw);
    return row ? anfApiRowToUi(row) : null;
  });
}
function anfDeleteApi(id) {
  return anfApiFetch('/api/v1/ccintern/anfragen/' + encodeURIComponent(String(id)), { method: 'DELETE' });
}
...
function renderAnfragenDom(){
  ...
  el.innerHTML=ANF_DATEN.map(a=>{
    ...
    const idLine=(a.anfragen_nr&&String(a.anfragen_nr).trim()!==''?a.anfragen_nr:a.id);
    ...
    const ico=a.leistungLabel&&a.leistungLabel.split(' ')[0]?a.leistungLabel.split(' ')[0]:'📋';
    return '<div onclick="anfOpenDetail(\''+a.id+'\')" style="display:flex;align-items:center;gap:10px;padding:12px 16px;border-bottom:1px solid var(--border);cursor:pointer;background:'+(isActive?'var(--green-l)':'#fff')+';transition:background .1s;">'
      +'<div style="width:36px;height:36px;border-radius:9px;background:'+(isActive?'var(--green)':'var(--green-l)')+';display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">'+ico+'</div>'
      +'<div style="flex:1;min-width:0;">'
        +'<div style="font-size:11px;font-weight:700;color:var(--text3);">'+idLine+' · '+a.erstellt+'</div>'
...
function anfSendenDirect(id,kanal){
  ...
  if(kanal==='whatsapp'){
    ...
    showToast('💬 WhatsApp · '+(a.anfragen_nr||a.id));
  } else {
    window.open('mailto:'+a.kontakt+'?subject='+encodeURIComponent('Angebot CC Werbung – '+(a.anfragen_nr||a.id))+'&body='+encodeURIComponent(text),'_blank');
    showToast('📤 E-Mail · '+(a.anfragen_nr||a.id));
  }
  function applyAngebotStatus() {
    a.status='angebot';
    renderAnfragen();
    anfOpenDetail(id);
  }
  if (window.__CCINTERN_COCKPIT_MOUNT__ && anfIsApiUuid(id)) {
    anfPutUiRowApi(a, 'in_bearbeitung')
      .then(function (ui) {
      ...
      .catch(function (err) {
        if (typeof showToast === 'function') {
          showToast('⚠ Status (Angebot): ' + (err && err.message ? err.message : String(err)));
        }
        applyAngebotStatus();
      });
    return;
...
function anfStatus(id,s){
  const a=ANF_DATEN.find(x=>x.id===id); if(!a) return;
  var apiSt = anfApiStatusFromUi(s);
  function apply() {
    a.status = s;
    renderAnfragen();
    anfOpenDetail(id);
    showToast(s==='gewonnen'?'🎉 '+(a.anfragen_nr||id)+' gewonnen!':'✕ '+(a.anfragen_nr||id)+' abgelehnt');
  }
  if (window.__CCINTERN_COCKPIT_MOUNT__ && anfIsApiUuid(id)) {
    anfPutUiRowApi(a, apiSt)
      .then(function (ui) {
        if (ui) Object.assign(a, ui);
        a.status = s;
        renderAnfragen();
...
(function anfWrapLoeschenForApi() {
  var tries = 0;
  function tick() {
    var orig = window.anfLoeschen;
    if (typeof orig !== 'function') {
      if (++tries < 160) setTimeout(tick, 50);
      return;
    }
    if (orig.__anfApiWrapped) return;
    window.anfLoeschen = function (id) {
      if (typeof ANF_DATEN === 'undefined') return;
      var anf = ANF_DATEN.find(function (x) {
        return x.id === id;
      });
      if (!anf || typeof ccInternConfirm !== 'function') return orig(id);
      ccInternConfirm(
        'Anfrage "' +
          (anf.anfragen_nr || id) +
          '" von ' +
        ...
        function () {
          function spliceLocal() {
            var idx = ANF_DATEN.findIndex(function (x) {
              return x.id === id;
            });
            if (idx !== -1) ANF_DATEN.splice(idx, 1);
            if (typeof AnfragenService !== 'undefined' && AnfragenService.save) AnfragenService.save();
            if (typeof window.renderAnfragen === 'function') window.renderAnfragen();
            if (typeof showToast === 'function') showToast('🗑 Anfrage gelöscht: ' + (anf.anfragen_nr || id));
          }
          if (window.__CCINTERN_COCKPIT_MOUNT__ && anfIsApiUuid(id)) {
            anfDeleteApi(id)
              .then(function () {
    ...
    window.anfLoeschen.__anfApiWrapped = true;
  }
  setTimeout(tick, 0);
})();
